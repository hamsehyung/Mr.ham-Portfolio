    ...
    <div style='text-align:center;padding:50px 0 20px'>
        <div style='margin:0 auto 12px;width:72px;height:72px;'>
            <svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
                <rect width="100" height="100" rx="20" fill="#fff0f0"/>
                <!-- 얇은 상승 차트 선 -->
                <polyline points="6,86 26,60 46,73 70,27 89,39"
                          stroke="#e03030" stroke-width="5" fill="none"
                          stroke-linecap="round" stroke-linejoin="round"/>
                <!-- 자연스러운 채움 화살표 (꺾임 없이 삼각형으로) -->
                <polygon points="78,13 96,37 70,37" fill="#e03030"/>
            </svg>
        </div>
...
st.title("📈 Mr.Ham  |  24Hr AI 포트폴리오 매니저 v14.0")
...
<script>
/* ── 빨간 상승 차트 파비콘 주입 ─────────────────────────────────────────
   Streamlit 기본 파비콘(파란색 📈)을 빨간 상승 차트 SVG로 교체.
   iframe 내부에서 window.parent.document의 <head>에 직접 삽입.
─────────────────────────────────────────────────────────────────────── */
(function setRedFavicon(){
    var doc = window.parent ? window.parent.document : document;
    /* SVG 파비콘 — 빨간 상승 차트 (한국증시 빨간=상승) */
    var svgRaw = [
        '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100">',
        '<rect width="100" height="100" rx="20" fill="%23fff0f0"/>',
        '<polyline points="6,86 26,60 46,73 70,27 89,39"',
        ' stroke="%23e03030" stroke-width="5" fill="none"',
        ' stroke-linecap="round" stroke-linejoin="round"/>',
        '<polygon points="78,13 96,37 70,37" fill="%23e03030"/>',
        '</svg>'
    ].join('');
    var href = 'data:image/svg+xml,' + svgRaw;
