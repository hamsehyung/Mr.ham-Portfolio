...
_NOISE_KEYWORDS = [
    # 날씨/기상
    "날씨", "기상", "소나기", "장마", "태풍", "폭염", "한파", "미세먼지",
    "내일 날씨", "주말 날씨", "무더위", "더위 계속", "더위 이어",
    # 스포츠
    "야구", "축구", "농구", "배구", "테니스", "골프 대회", "올림픽", "월드컵",
    # 연예·라이프
    "드라마", "영화 개봉", "아이돌", "K-pop", "콘서트", "시상식 레드카펫",
    "맛집", "여행지", "관광",
]

def _is_noise_article(title: str) -> bool:
    """투자 관련성 없는 노이즈 기사 여부 판단"""
    return any(kw in title for kw in _NOISE_KEYWORDS)
...
# 수급 API 전용 세션 — 앱 시작 시 1회 쿠키 획득 후 재사용
...
# 앱 로드 시 1회 쿠키 세팅 (NNB 등)
try:
    _NAVER_INV_SESSION.get("https://m.stock.naver.com/", timeout=4)
except Exception:
    pass
...
"""
    ...
    def _clean_report(report: str) -> str:
        ...
        for marker in STRIP_MARKERS:
            ...
            if idx > 200:                # 본문 최소 200자 이후에서만 제거 (오탐 방지)
...
st.title("📈 Mr.Ham  |  24Hr AI 포트폴리오 매니저 v11.0")
