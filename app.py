...
def save_last_report(report: str, date_str: str) -> None:
    ...
    idx = report.find("## 4.")
    opinions = report[idx:idx+8000] if idx >= 0 else report[:8000]  # [Fix-B] 10종목 이상 대응
...
def fetch_investor_trend_raw(ticker: str) -> list:
    ...
    @st.cache_data로 캐시 → 5일·20일 집계 모두 이 함수 재사용, HTTP 1회로 절감.
    [Fix-C] 정규식 완화: 6자리 영숫자 ETF(0186L0 등) 포함 처리
    """
    if not ticker: return []
    # 6자리 숫자 OR 앞4자리가 숫자인 6자리 영숫자 티커(KoAct ETF 등)만 허용
    is_kr = (re.match(r"^\d{6}$", ticker) or
...
def fetch_investor_trend(ticker: str, days: int = 5) -> dict:
    ...
    [Fix-C] 네이버 API 필드명 폴백 확장 (API 변경 대응)
    """
    ...
    fn = mn = 0
    for row in raw[:days]:
        # 네이버 API 필드명 여러 버전 폴백
        fn += si(row.get("foreignNetSale",
                 row.get("foreignPureBuyQuant",
                 row.get("fBuyAmount", row.get("foreign_net", 0)))))
        mn += si(row.get("organNetSale",
                 row.get("organPureBuyQuant",
                 row.get("instituteNetBuy", row.get("institution_net", 0)))))
...
"""
    ...
    if market_shock:
        ...
        prev_context = (
            ...
            f"[참고용 이전 의견]\n{prev_opinions[:2000] if prev_opinions else '없음'}"
        )
    elif prev_opinions:
        prev_context = (
