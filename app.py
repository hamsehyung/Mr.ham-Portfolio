...
def _build_gen_config(max_tokens, temperature):
    """
    [v32] Gemini 2.5 Flash 사고(thinking) 토큰이 max_output_tokens 예산을
    소모해 이어받기가 빈발하는 문제 해결.
    thinking_budget=0 으로 사고 비활성화 → 전체 예산을 실제 출력에 사용.
    SDK가 ThinkingConfig를 지원하지 않으면 기본 설정으로 안전 폴백.
    """
    base = dict(
    ...
    # 1차: thinking_config 지원 SDK
    try:
        _tc = genai.types.ThinkingConfig(thinking_budget=0)
        return genai.types.GenerationConfig(**base, thinking_config=_tc)
    except (AttributeError, TypeError):
        pass
    # 2차: 기본 설정 (thinking 제어 미지원 SDK)
