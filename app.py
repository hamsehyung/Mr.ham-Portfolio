...
def fetch_naver_fundamentals(ticker: str) -> dict:
    ...
    # ── [추가] 국내 주식 PEG 미수집 시 자체 계산 보완 (Fallback) ──
    # yfinance에서 PEG를 주지 않았더라도, PER과 ROE(또는 성장률)가 있다면 추정치를 만듭니다.
    if result.get("peg") is None and result.get("per"):
        # 1순위: PER / 매출성장률
        if result.get("rev_growth") and result["rev_growth"] > 0 and result["per"] > 0:
            result["peg"] = round(result["per"] / result["rev_growth"], 2)
        # 2순위: PER / ROE (이익률 기반 추정)
        elif result.get("roe") and result["roe"] > 0 and result["per"] > 0:
            result["peg"] = round(result["per"] / result["roe"], 2)
    ...
    for lbl,key in ACC_MAP.items():
        ...
        for info in portfolio.get(key,{}).values():
            ...
            if is_etf(nm):
                ...
                if any(k in nm.upper() for k in [x.upper() for x in FOREIGN_KEYS]): fe+=iv
                else: de+=iv
            else: sv+=iv
    ...
    if st.button("🔄 최신 데이터 수집 & AI 포트폴리오 분석 실행", type="primary", use_container_width=True):
        if not selected_model:
            ...
            try:
                ...
                # 👇 수정: 결과 저장 시간도 KST로 강제 지정
                st.session_state.report_time = datetime.datetime.now(KST).strftime("%Y-%m-%d %H:%M:%S")
