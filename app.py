"""
=============================================================
  📈 Mr.Ham — 24Hr AI 포트폴리오 매니저 v11.0 (국내상장주식용)
=============================================================
  🔐 보안 정책
...
def save_last_report(report: str, date_str: str) -> None:
    ...
    idx = report.find("## 4.")
    opinions = report[idx:] if idx >= 0 else report  # [Fix-Task2] 8000자 제한 철폐 — Gemini 컨텍스트 충분
    portfolio_with_meta = dict(portfolio)
...
def fetch_investor_trend_raw(ticker: str) -> list:
    """
    네이버 투자자 동향 원시 데이터를 20일치 한 번에 가져와 캐시.
    - pageSize=20 파라미터 추가 (응답 안정성)
    - API가 list / dict 어떤 형태로 와도 list 부분만 추출 (Gemini 진단 반영)
    """
    ...
    try:
        ...
        if isinstance(raw, list):
            return raw
        # Case 2: 딕셔너리로 포장된 경우 — 안쪽 리스트를 꺼냄
        if isinstance(raw, dict):
            for key in ("result", "list", "data", "items"):
                if key in raw and isinstance(raw[key], list):
                    return raw[key]
            # 위 key 없으면 dict 값 중 첫 번째 list 반환
            for v in raw.values():
...
def fetch_investor_trend(ticker: str, days: int = 5) -> dict:
    """
    fetch_investor_trend_raw(캐시)를 재사용해 n일 집계 반환.
    [Fix-Task1] Double Counting 버그 수정:
      한 row 내에 foreign 관련 키(foreignNetSale, foreignBuyQuant, foreignSellQuant 등)가
      여러 개 존재해도 1개의 값만 추출. 우선순위 기반 헬퍼 함수로 중복 합산 원천 차단.
    """
...
def display_portfolio(title: str, portfolio_dict: dict):
    ...
    for item_id, info in portfolio_dict.items():
        ...
        _memo_html = (f'<span style="font-size:11px;background:#e8f4fd;color:#1a6fb5;'
                       ...
                       f'vertical-align:middle">📌 {_memo}</span>') if _memo else ""
        st.markdown(f"""
<div style="background:#f8f9fa;padding:14px 16px;border-radius:10px;
  ...
  <h5 style="margin:0 0 6px 0"><strong>{info["name"]}</strong>
    <span style="font-size:13px;color:gray">&nbsp;({ticker}) | {info["qty"]:,}주</span>
    {_memo_html}</h5>
...
def build_portfolio_text(portfolio: dict) -> str:
    ...
    for tk,m in merged.items():
        ...
        if inv5 or inv20:
            fn5  = (inv5  or {}).get("foreign_net",     0)
            mn5  = (inv5  or {}).get("institution_net", 0)
            fn20 = (inv20 or {}).get("foreign_net",     0)
            mn20 = (inv20 or {}).get("institution_net", 0)
            inv_str = (f"외국인 5일:{_arrow(fn5)}{abs(fn5):,}주"
                       f"·20일:{_arrow(fn20)}{abs(fn20):,}주 | "
                       f"기관 5일:{_arrow(mn5)}{abs(mn5):,}주"
                       f"·20일:{_arrow(mn20)}{abs(mn20):,}주")
        else:
            # 국내 6자리 숫자 티커가 아닌 경우(해외 ETF 등) N/A 표시
            inv_str = "N/A (해외종목 또는 API 미지원)"
...
def call_gemini(api_key, model_name, prompt, max_tokens,
    ...
    while truncated and cont_count<max_cont:
        ...
        # ── 이미 작성된 섹션 감지 → 남은 섹션만 이어받기 ──
        # [Task1] ## 8 추가 + 원본 prompt 재주입으로 규칙 기억 유지
        _allowed = [
            "## 4. 📊 보유 종목 분석",
            "## 5. 💡 신규 추천 종목 (10루타 후보)",
            "## 6. ⚖️ 포트폴리오 리밸런싱 제언",
        ...
        _written   = [s for s in _allowed if s in full_text]
        ...
        # [Task1] 출력 규칙+템플릿 부분만 발췌: '🚨 [출력 준수 규칙]' 마커 이후를 추출.
        # - prompt[:2000] 제거 → 앞 데이터에 묻혀 템플릿을 못 보는 환각 완전 차단
        # - 마커가 없으면 prompt 전체 사용 (안전 폴백)
        _rules_marker = "🚨 [출력 준수 규칙]"
        _rules_idx    = prompt.find(_rules_marker)
        _prompt_rules = prompt[_rules_idx:] if _rules_idx >= 0 else prompt

        cont_prompt=f"""[리포트 이어받기 — 원본 지시사항 재확인]
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
당신은 직전에 중단된 포트폴리오 분석 리포트를 이어 작성 중입니다.
아래 [원본 지시사항]의 모든 규칙(페르소나, 투자 원칙, 출력 형식)이 이어받기에도 동일하게 적용됩니다.

[원본 지시사항 — 규칙 및 출력 템플릿 (## 4 ~ ## 8)]
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
{_prompt_rules}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

[이 리포트의 허용 섹션 목록 — ## 4 ~ ## 8 이 전부]
  ## 4. 📊 보유 종목 분석
  ## 5. 💡 신규 추천 종목 (10루타 후보)
  ## 6. ⚖️ 포트폴리오 리밸런싱 제언
  ## 7. 📅 핵심 모니터링 지표 (펀더멘털 훼손 조기 경보)
...
⛔ 절대 금지 (이어받기 중 가장 흔한 실수):
  - ## 9, ## 10, ## 11 등 위 목록에 없는 번호의 새 목차(섹션) 창조 금지
  - 이미 작성된 섹션의 재작성·요약·반복 금지
  - 인사말·소개문 삽입 금지
  - ## 8 이후 어떤 섹션도 추가하지 마세요

✅ 이미 작성 완료된 섹션:
{_written_str}

📝 지금 작성해야 할 남은 섹션 (이것만 작성):
{_remaining_str}

[중단 직전 마지막 내용 — 여기서 바로 이어쓰세요]
──────────────────────────────────────
{full_text[-1500:].strip()}
──────────────────────────────────────
위 내용이 끊긴 부분부터 자연스럽게 이어서, [남은 섹션]만 완성 후 ## 8 결론으로 마무리하세요."""
        try:
...
이어받기(continuation) 시에도 동일하게 적용됩니다.
"""
