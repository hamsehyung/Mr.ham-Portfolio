...
MARKET_TICKERS = {
    ...
    "미 국채 10년물": "^TNX",   # [추가] 거시경제 금리 판단 핵심 지표
}
    ...
    if not re.match(r"^\d{6}$", ticker):
        if HAS_YFINANCE:
            try:
                ...
                # ── [추가] 미래 가치·성장성 지표 ──────────────────────
                # Forward PER: 향후 12개월 예상이익 기준 (trailingPE보다 미래 지향적)
                result["forward_per"]   = info.get("forwardPE")
                # PEG Ratio: PER ÷ 이익성장률 → 1 이하면 성장 대비 저평가
                result["peg"]           = info.get("pegRatio")
                # 매출액 증가율 (YoY, % 단위로 변환)
                rev_g = info.get("revenueGrowth")
                result["rev_growth"]    = round(rev_g*100, 1) if rev_g else None
                # 부채비율: 재무 건전성 (낮을수록 안정)
                result["debt_equity"]   = info.get("debtToEquity")
                # 잉여현금흐름 (억원 단위 변환, 주주환원·재투자 여력)
                fcf_raw = info.get("freeCashflow")
                result["fcf_억"]        = int(fcf_raw/1e8) if fcf_raw else None
            except: pass
        return result
    ...
    for tk,m in merged.items():
        ...
        if is_etf(m["name"]):
            ...
            fp=" | ".join(filter(None,[
                ...
                # ── [추가] 미래 가치·성장성 지표 ──────────────────────
                # Forward PER: 현재 PER과 비교해 고평가 여부 판단
