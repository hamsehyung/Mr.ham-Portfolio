...
# ═══════════════════════════════════════════════════════════
#  [v11.3 핵심수정] 캐시를 '전역변수'에서 'session_state'로 이전
#  ──────────────────────────────────────────────────────────
#  버그: Streamlit 은 상호작용마다 스크립트 전체를 다시 실행한다.
#        따라서 모듈 최상단의 `_TS_CACHE = {}` 는 매 rerun 마다
#        빈 dict 로 초기화된다(실측 확인).
#        → '현재가 새로고침'(=warmup 실행) 직후엔 같은 run 안에서
#          캐시가 채워져 정상 표시되지만,
#          'AI 분석 실행' 후 rerun 되면 캐시가 통째로 사라지고
#          warmup 은 session_state 플래그 때문에 건너뛰어
#          전 종목이 (0,0,0) → 회색 '미수집' 이 됐다.
#  해결: dict 객체를 session_state 에 보관하고 그 '참조'를 사용한다.
#        session_state 는 rerun 을 넘어 유지되며,
#        백그라운드 스레드도 참조를 통해 안전하게 읽고 쓸 수 있다.
#        (스레드에서 금지되는 것은 st.* API 호출이지, 객체 참조가 아님)
# ═══════════════════════════════════════════════════════════
def _sv_dict(key: str) -> dict:
    """session_state 에 보관된 dict 를 가져오거나 새로 만든다."""
    try:
        if key not in st.session_state:
            st.session_state[key] = {}
        return st.session_state[key]
    except Exception:
        # session_state 를 쓸 수 없는 문맥(스레드 등) — 임시 dict 로 폴백
        return globals().setdefault("_fallback_" + key, {})

_TS_CACHE: dict = _sv_dict("_ts_cache_store")
...
# ═══════════════════════════════════════════════════════════
#  [v11.2] 최종 성공값(last-good) 저장소
#  ──────────────────────────────────────────────────────────
#  문제: TTL(300초) 캐시만 쓰면, AI 분석처럼 오래 걸리는 작업 뒤
#        캐시가 만료되어 전 종목이 (0,0,0) → 회색 '미수집'으로 표시됐다.
#  해결: 한 번이라도 성공한 가격은 TTL 과 무관하게 별도 보관하고,
#        신선한 캐시가 없으면 이 값을 '이전 시세'로 표시한다.
#        → 화면이 비는 일이 없고, 데이터 출처도 사용자에게 명시된다.
# ═══════════════════════════════════════════════════════════
# [v11.3] last-good 도 session_state 에 보관해야 rerun 후에도 살아남는다.
_LAST_GOOD: dict = _sv_dict("_last_good_store")   # {ticker: (value, ts)}
_LAST_GOOD_LOCK = _threading.Lock()
FRESH_TTL = 300                # 이 시간 안이면 '실시간'


def _remember_good(ticker: str, val: tuple) -> None:
    """성공한 시세를 영구 보관 (TTL 없음)."""
    if val and val[0] > 0:
        with _LAST_GOOD_LOCK:
            _LAST_GOOD[ticker] = (val, _time_mod.time())


def _cached_stock_data_only(ticker: str) -> tuple:
    """
    [v31] 렌더링 전용 — 네트워크 호출을 절대 하지 않음.
    [v11.2] 신선 캐시가 없으면 last-good 으로 폴백 (0 반환 금지).
    """
    ...
    with _TS_CACHE_LOCK:
        ...
        if hit is not None:
            val, ts = hit
            if _time_mod.time() - ts < FRESH_TTL and val and val[0] > 0:
                _remember_good(ticker, val)
                return val
    # 신선한 값이 없으면 마지막 성공값 사용
    with _LAST_GOOD_LOCK:
        old = _LAST_GOOD.get(ticker)
    return old[0] if old else (0, 0, 0)


def _price_age(ticker: str) -> float:
    """마지막 성공 시세의 경과 시간(초). 없으면 -1."""
    key = ("get_stock_data", (ticker,))
    with _TS_CACHE_LOCK:
        hit = _TS_CACHE.get(key)
    if hit and hit[0] and hit[0][0] > 0:
        return _time_mod.time() - hit[1]
    with _LAST_GOOD_LOCK:
        old = _LAST_GOOD.get(ticker)
    return (_time_mod.time() - old[1]) if old else -1.0

# ═══════════════════════════════════════════════════════════
#  v11 신규 모듈 (스크리닝·공시·추천기록)
#  ──────────────────────────────────────────────────────────
#  [v11.2] 임포트 성공만으로는 부족하다.
#    app.py 만 갱신하고 모듈 파일이 이전 버전으로 남아 있으면
#    실행 중 AttributeError 로 앱 전체가 죽는다(실제 발생 사례).
#    → 임포트 시점에 '필요한 함수가 전부 있는지' 검사하고,
#      하나라도 없으면 그 모듈 기능만 끈다. 앱은 계속 동작한다.
# ═══════════════════════════════════════════════════════════
# 필수 함수 목록. 문자열 대신 (함수명, [필수 파라미터...]) 튜플도 쓸 수 있다.
# [v11.3] 함수 '존재'만 보면, 함수는 있는데 파라미터가 없는 구버전을 통과시켜
#   실행 중 TypeError(unexpected keyword argument)로 실패한다(실제 발생).
#   → 시그니처까지 검사해 임포트 시점에 잡는다.
_MODULE_SPEC = {
    "mrham_screener": ("SCREENER", [
        ("run_screening", ["fund_fetcher", "fund_pool",
                           "price_fetcher", "flow_fetcher"]),
        "build_candidate_context",
        "enrich_fundamentals"]),
    "mrham_dart":     ("DART", [
        "validate_api_key", "build_corp_code_map", "fetch_disclosures",
        "analyze_disclosures", "build_disclosure_context",
        "get_critical_alerts"]),
    "mrham_track":    ("TRACK", [
        "load_recommendations", "save_recommendations",
        "parse_recommendations", "evaluate_recommendations",
        "build_performance_summary"]),
    "mrham_prompt":   ("MPROMPT", [
        "build_critical_alert_text", "build_recommendation_prompt"]),
}

_MODULE_STATUS: dict = {}    # {모듈명: "ok" | "missing" | "outdated:함수1,함수2"}

def _load_module(modname: str, required: list):
    """
    모듈을 임포트하고 '필수 함수 + 필수 파라미터'까지 확인.
    Returns: (모듈객체, 사용가능여부)

    required 항목은 두 형태를 지원한다.
      "func_name"                      → 함수 존재만 확인
      ("func_name", ["param1", ...])   → 파라미터 존재까지 확인
    """
    try:
        mod = __import__(modname)
    except Exception:
        _MODULE_STATUS[modname] = "missing"
        return None, False

    import inspect as _insp
    absent = []
    for item in required:
        fname, params = (item, []) if isinstance(item, str) else item
        ...
        if params:
            try:
                ...
                miss = [p for p in params if p not in sig.parameters]
                if miss:
                    absent.append(f"{fname}({', '.join(miss)})")
            except Exception:
                pass          # 시그니처를 못 읽으면 통과시킨다

    if absent:
        _MODULE_STATUS[modname] = "outdated:" + ",".join(absent)
        return mod, False
    _MODULE_STATUS[modname] = "ok"
    return mod, True

SCREENER, HAS_SCREENER = _load_module("mrham_screener", _MODULE_SPEC["mrham_screener"][1])
DART,     HAS_DART_MOD = _load_module("mrham_dart",     _MODULE_SPEC["mrham_dart"][1])
TRACK,    HAS_TRACK    = _load_module("mrham_track",    _MODULE_SPEC["mrham_track"][1])
MPROMPT,  HAS_MPROMPT  = _load_module("mrham_prompt",   _MODULE_SPEC["mrham_prompt"][1])


def _module_problems() -> list:
    """사용자에게 보여줄 모듈 문제 목록."""
    out = []
    for modname, st_ in _MODULE_STATUS.items():
        if st_ == "ok":
            continue
        if st_ == "missing":
            out.append((modname, f"`{modname}.py` 파일이 없습니다."))
        elif st_.startswith("outdated:"):
            fns = st_.split(":", 1)[1]
            out.append((modname,
                        f"`{modname}.py` 가 이전 버전입니다. (누락 함수: {fns})"))
    return out
...
STAGE1_MAX_TOKENS       = 5000   # [v34] 사고(~2k)+매크로 출력(~1k) 수용
STAGE2_MAX_TOKENS       = 16384  # [v34] 사고(최대4k)+14종목 출력(~8k)을 한 번에 수용
                                 #   → 이어받기 제거하면서 사고 기반 분석 품질 유지
CONTINUATION_MAX_TOKENS = 10000  # 이어받기 발생 시에도 사고+출력 충분
...
#  포트폴리오 UI
# ═══════════════════════════════════════════════════════════
def _freshness_badge(missing: bool, stale: bool, age: float) -> str:
    """
    [v11.2] 시세 신선도 배지.
      · 미수집  → 주황 (평단가로 대체 표시)
      · 이전시세 → 회색 (last-good 값, 경과 시간 표기)
      · 실시간  → 배지 없음
    """
    if missing:
        return (' <span style="color:#e67e22;font-size:11px">'
                '(미수집·평단표시)</span>')
    if stale:
        mins = int(age // 60)
        when = f"{mins}분 전" if mins < 60 else f"{mins//60}시간 전"
        return (f' <span style="color:#8a8a8a;font-size:11px">'
                f'({when} 시세)</span>')
    return ""
...
def display_portfolio(title: str, portfolio_dict: dict):
    ...
    for item_id, info in portfolio_dict.items():
        ...
        # [v11.2] 신선도 3단계 — 실시간 / 이전시세 / 미수집
        #   기존엔 캐시 만료 시 곧바로 평단가로 대체돼 전 종목이 회색이 됐다.
        #   이제 last-good 폴백이 있어, 값이 있으면 '몇 분 전' 시세로 표시한다.
        _age = _price_age(ticker)
        _price_missing = (curr == 0)
        _price_stale   = (not _price_missing) and (_age > FRESH_TTL)
...
<div style="background:#f8f9fa;padding:14px 16px;border-radius:10px;
  ...
  <div style="font-size:14px;line-height:1.9">
    평균단가: {avg_price:,}원 &nbsp;→&nbsp; <b>현재가: {curr:,}원{_freshness_badge(_price_missing, _price_stale, _age)}</b><br>
...
def _warmup_portfolio_cache(portfolio: dict) -> None:
    ...
    def _fetch_one(tk):
        try:
            _v = get_stock_data(tk)
            _remember_good(tk, _v)     # [v11.2] 성공 시세를 영구 보관
        except: pass
...
# ═══════════════════════════════════════════════════════════
#  v11 — Layer 0~2 스크리닝 / DART 공시 / 추천기록 래퍼
# ═══════════════════════════════════════════════════════════
@st.cache_data(ttl=3600, max_entries=2, show_spinner=False)
def run_full_screening(top_n: int = 15, momentum_pool: int = 250) -> dict:
    """전종목 스크리닝 — 1시간 캐시. 실패해도 앱은 계속 동작."""
    if not HAS_SCREENER:
        return {"top": None, "error": "screener 모듈 미설치", "stats": {}, "source": None,
                "as_of": today_kst().strftime("%Y-%m-%d")}
    try:
        # [v11.3] 유니버스 소스(FDR StockListing)는 PER/PBR 을 제공하지 않는다.
        #   → 앱에 이미 있는 네이버 펀더멘털 수집기를 주입해 Layer 1.5 에서 보강.
        # 구버전 모듈에도 안전하도록 지원하는 인자만 골라서 전달한다.
        import inspect as _i
        _params = _i.signature(SCREENER.run_screening).parameters
        _kw = {"top_n": top_n, "momentum_pool": momentum_pool}
        if "fund_fetcher" in _params:
            _kw["fund_fetcher"] = fetch_naver_fundamentals
        if "fund_pool" in _params:
            _kw["fund_pool"] = 300
        return SCREENER.run_screening(**_kw)
    except Exception as e:
        return {"top": None, "error": str(e), "stats": {}, "source": None,
                "as_of": today_kst().strftime("%Y-%m-%d")}


@st.cache_data(ttl=86400, max_entries=1, show_spinner=False)
def get_dart_corp_map(dart_key: str) -> dict:
    """DART 종목코드 매핑 — 하루 1회만 수집(용량이 큼)."""
    if not HAS_DART_MOD or not dart_key:
        return {}
    try:
        return DART.build_corp_code_map(dart_key)
    except Exception:
        return {}


@st.cache_data(ttl=900, max_entries=3, show_spinner=False)
def fetch_my_disclosures(dart_key: str, tickers_tuple: tuple, days: int = 3) -> list:
    """보유 종목 공시 — 15분 캐시."""
    if not HAS_DART_MOD or not dart_key:
        return []
    try:
        cmap = get_dart_corp_map(dart_key)
        codes = [cmap[t] for t in tickers_tuple if t in cmap]
        res = DART.fetch_disclosures(dart_key, codes or None, days=days)
        if res["status"] != "ok":
            return []
        return DART.analyze_disclosures(res["items"], my_tickers=set(tickers_tuple))
    except Exception:
        return []


def get_my_tickers() -> set:
    """보유 종목코드 집합"""
    out = set()
    try:
        pf = st.session_state.get("portfolio") or {}
        for key in ACC_MAP.values():
            for info in pf.get(key, {}).values():
                tk = str(info.get("ticker","")).strip().upper()
                # [v11.1] 신형 ETF/ETN 코드(0155M0, 0186L0 등)는 영숫자 6자리.
                # 기존 ^\d{6}$ 는 이들을 통째로 누락시켜 DART 조회에서 빠졌음.
                if re.match(r"^[0-9A-Z]{6}$", tk):
                    out.add(tk)
    except Exception:
        pass
    return out
...
def _build_gen_config(max_tokens, temperature, thinking_budget=4096):
    """
    [v34] google-genai 신규 SDK — thinking_config 정식 지원.
    thinking_budget 의미:
      · 0    = 사고 완전 비활성화 (빠르지만 분석 깊이 ↓)
      · 양수 = 사고 토큰 상한 (기본 4096 — 분석 품질 유지하며 토큰 예산 보호)
      · -1   = 동적(모델 자동, 최대 24576 — 토큰 과다 소모로 이어받기 유발)
    품질(정확한 의견 도출)을 위해 사고는 유지하되, 상한을 둬서 이어받기를 방지.
    """
    base = dict(
    ...
    try:
        base["thinking_config"] = genai_types.ThinkingConfig(thinking_budget=thinking_budget)
...
def _call_single(client, model_name, prompt, max_tokens, temperature=0.0):
    ...
    # [v34] 신규 SDK는 빈/차단/사고전용 응답 시 resp.text가 None → "" 로 방어
    text = resp.text or ""
    return text, _is_truncated(resp)
...
def call_gemini(api_key, model_name, prompt, max_tokens,
    ...
    for attempt,model in enumerate(candidates):
        ...
        # [v34] 503(과부하)·429(할당량) 발생 시 동일 모델로 지수 백오프 재시도
        _retry503 = 0
        while True:
            try:
                ...
                if "404" in msg or "not found" in msg:
                    ...
                    # 서버 과부하 — 동일 모델로 최대 3회 지수 백오프 재시도
                    if _retry503 < 3:
                        _retry503 += 1
                        _wait_s = 3 * _retry503  # 3 → 6 → 9초
                        if status_ph:
                            status_ph.text(f"⏳ [{stage_label}] 서버 과부하(503) — {_wait_s}초 후 재시도 ({_retry503}/3)...")
                        time.sleep(_wait_s)
                        continue  # 같은 모델 재시도
                    else:
                        break  # 3회 실패 → 다음 모델로
                elif "429" in msg or "quota" in msg or "rate" in msg:
                    if next_m: st.toast(f"⚠️ {model} 할당량 소진 → {next_m} 전환",icon="🔄"); time.sleep(2)
                    break  # 다음 모델로
                else:
                    raise e
        if full_text:  # 성공 시 모델 루프 탈출
            break
    else:
        raise RuntimeError(f"[{stage_label}] 모든 모델 시도 실패 (서버 과부하 지속 — 잠시 후 재시도 권장)")
    cont_count=0
...
with st.sidebar:
    ...
    # ── v11: DART 공시 연동 (선택) ────────────────────────
    # [v11.2 수정] 기존엔 문자열이 비어있지 않기만 하면 '연동됨'으로 표시했다.
    #   → 아무 문자나 넣어도 성공처럼 보여 오해를 유발.
    #   이제 형식(40자리 hex) 검사 + DART 서버 실제 조회로 검증한다.
    with st.expander("📢 DART 공시 연동 (선택)", expanded=False):
        st.caption("보유 종목의 유상증자·자사주매입 등을 실시간 감지합니다.")
        _dk_in = st.text_input(
            "DART API Key", type="password", placeholder="40자리 키",
            value=st.session_state.get("dart_key_input_val",""),
            key="dart_key_input",
            help="opendart.fss.or.kr 에서 무료 발급 (일 20,000건)")

        # 입력이 바뀌면 검증 결과를 초기화 (미검증 상태로)
        if _dk_in != st.session_state.get("dart_key_input_val",""):
            st.session_state.dart_key_input_val = _dk_in
            st.session_state.dart_key       = ""     # 검증 전엔 사용 안 함
            st.session_state.dart_validated = None

        _dv = st.session_state.get("dart_validated")

        if _dk_in and _dv is None:
            if st.button("🔑 키 검증하기", key="btn_dart_verify",
                         use_container_width=True):
                if not HAS_DART_MOD:
                    st.session_state.dart_validated = {
                        "valid": False, "status": "error",
                        "message": "mrham_dart.py 모듈을 찾을 수 없습니다."}
                elif not hasattr(DART, "validate_api_key"):
                    # [v11.2] 버전 불일치 방어 —
                    #   app.py 만 갱신하고 mrham_dart.py 를 갱신하지 않으면
                    #   AttributeError 로 앱 전체가 죽던 문제를 안내 메시지로 대체.
                    st.session_state.dart_validated = {
                        "valid": False, "status": "outdated_module",
                        "message": "mrham_dart.py 가 이전 버전입니다. "
                                   "app.py 와 함께 최신 파일로 교체해주세요."}
                else:
                    try:
                        with st.spinner("DART 서버에 확인 중..."):
                            _res = DART.validate_api_key(_dk_in)
                    except Exception as _e:
                        _res = {"valid": False, "status": "error",
                                "message": f"검증 중 오류: {type(_e).__name__}"}
                    st.session_state.dart_validated = _res
                    if _res.get("valid"):
                        st.session_state.dart_key = _dk_in
                st.rerun()
            st.caption("⚠️ 아직 검증 전입니다. 위 버튼을 눌러 확인해주세요.")

        elif _dv is not None:
            if _dv["valid"]:
                st.success("✅ DART 연동됨 — 서버 검증 완료")
            else:
                _icon = {"bad_format": "✏️", "invalid_key": "🔑",
                         "no_permission": "🚫", "network": "📡",
                         "outdated_module": "📦"}.get(_dv["status"], "❌")
                st.error(f"{_icon} 연동 실패 — {_dv['message']}")
                if _dv["status"] == "outdated_module":
                    st.caption("GitHub 저장소에 `mrham_dart.py` 최신본을 "
                               "업로드한 뒤 앱을 재시작해주세요.")
                if _dv["status"] == "bad_format":
                    st.caption("DART 키는 소문자·숫자로 이루어진 40자리입니다.")
                elif _dv["status"] == "network":
                    st.caption("잠시 후 다시 시도해주세요. 공시 기능만 비활성화되고 "
                               "나머지 기능은 정상 동작합니다.")
                if st.button("🔄 다시 검증", key="btn_dart_retry",
                             use_container_width=True):
                    st.session_state.dart_validated = None
                    st.rerun()

        if not _dk_in:
            st.caption("👉 [무료 키 발급](https://opendart.fss.or.kr)")

    st.divider()

    # ── 데이터 수집 상태 ──────────────────────────────────
    # [v11] pykrx 는 '필수'가 아니라 '선택' 항목으로 분리.
    #   · matplotlib/numpy>=2.0 을 강제로 끌어와 Streamlit Cloud 설치 실패 유발
    #   · pykrx 1.2.8+ 는 KRX_ID/KRX_PW 로그인 필요 → Cloud 에서 어차피 미동작
    #   → 미설치를 '경고'가 아닌 '정상'으로 표시해야 오해가 없음
    st.subheader("📡 데이터 수집 상태")

    _core = [("FinanceDataReader", HAS_FDR), ("feedparser", HAS_FEEDPARSER),
             ("yfinance", HAS_YFINANCE)]
    _core_missing = [x for x, y in _core if not y]

    if _core_missing:
        st.error(f"❌ 필수 미설치: `{', '.join(_core_missing)}`")
        st.caption("requirements.txt 를 확인해주세요. 앱 기능이 제한됩니다.")
    else:
        st.success("✅ 필수 라이브러리 정상")

    # [v11.2] 모듈 파일 버전 불일치 감지 — 가장 흔한 배포 실수
    _probs = _module_problems()
    if _probs:
        st.error("📦 모듈 파일 문제 — 기능이 비활성화됨")
        for _m, _msg in _probs:
            st.caption(f"· {_msg}")
        st.caption("👉 GitHub 저장소에 해당 `.py` 파일 최신본을 "
                   "`app.py` 와 **같은 폴더**에 업로드한 뒤 앱을 재시작하세요.")

    # 활성 모듈 표시
    _opt = []
    if HAS_SCREENER: _opt.append("스크리닝")
    if HAS_DART_MOD and st.session_state.get("dart_key"): _opt.append("DART공시")
    if HAS_TRACK:    _opt.append("추천기록")
    if _opt:
        st.caption(f"🔍 활성 모듈: {' · '.join(_opt)}")

    if not HAS_PYKRX:
        st.caption("ℹ️ 수급 데이터(pykrx)는 선택 항목입니다. "
                   "미설치 상태가 정상이며, 가치·퀄리티·모멘텀 3개 팩터로 랭킹이 동작합니다.")
    else:
        st.caption("✅ pykrx 활성 — 수급 팩터 포함 4팩터 랭킹")
    ...
    st.divider()
    st.caption("캐시 — 주가 5분 | 뉴스 10분 | 펀더멘털 1시간 | 스크리닝 1시간")
    if not HAS_PYKRX:
        st.caption("수급 팩터는 중립(50점) 처리되며, 나머지 3개 팩터로 랭킹이 산출됩니다.")
...
# ══════════════════════════════
with tab_pf:
    # [v31] calc_totals 전에 warmup 먼저 실행
    # [v11.2 수정] 기존엔 '세션당 1회' 영구 플래그라, 캐시(300초)가 만료돼도
    #   재수집이 일어나지 않아 AI 분석처럼 오래 걸리는 작업 뒤 전 종목이
    #   회색 '미수집'으로 변했다. → 시간 기반으로 바꿔 만료되면 자동 재워밍업.
    _warm_at   = st.session_state.get("_pf_warmed_at", 0.0)
    _warm_age  = _time_mod.time() - _warm_at

    # [v11.3] 안전장치 — 시간 조건과 무관하게 '캐시가 비었으면' 반드시 재수집.
    #   플래그만 믿으면, 어떤 이유로든 캐시가 사라졌을 때
    #   warmup 을 영영 건너뛰어 전 종목이 회색이 된다.
    _held = {info.get("ticker","") for _k in ACC_MAP.values()
             for info in st.session_state.portfolio.get(_k, {}).values()
             if info.get("ticker")}
    _have = 0
    if _held:
        with _LAST_GOOD_LOCK:
            _have = sum(1 for t in _held if t in _LAST_GOOD)
    _cache_empty = bool(_held) and _have == 0

    _need_warm = (_warm_age > FRESH_TTL
                  or not st.session_state.get("_pf_warmed")
                  or _cache_empty)

    if _need_warm:
        _first = not st.session_state.get("_pf_warmed")
        with st.spinner("📡 시세 수집 중..." if _first else "📡 시세 갱신 중..."):
            _warmup_portfolio_cache(st.session_state.portfolio)
        st.session_state["_pf_warmed"]    = True
        st.session_state["_pf_warmed_at"] = _time_mod.time()
    ...
    with _rc2:
        if st.button("🔄 현재가 새로고침", key="refresh_prices", use_container_width=True):
            ...
            st.session_state["_pf_warmed_at"] = 0.0
            st.rerun()
    ...
    # ═══════════════════════════════════════════════════════
    #  [v11.1] 주가 수집 진단 — '미수집·평단표시' 원인 자가 확인
    # ═══════════════════════════════════════════════════════
    with st.expander("🩺 주가 수집 진단 (현재가가 '미수집'으로 뜰 때)", expanded=False):
        st.caption("보유 종목 중 3개를 골라 데이터 소스별로 실제 응답을 확인합니다.")
        if st.button("🔬 진단 실행", key="btn_diag", use_container_width=True):
            _tks = []
            for _k in ACC_MAP.values():
                for _i in st.session_state.portfolio.get(_k, {}).values():
                    _t = str(_i.get("ticker","")).strip()
                    if _t and _t not in _tks:
                        _tks.append(_t)
            _tks = _tks[:3]
            ...
            if not _tks:
                ...
                for _tk in _tks:
                    ...
                    _s = _t_mod.time()
                    try:
                        _c, _h, _l = _naver_price(_tk)
                        _row["Naver"] = f"✅ {_c:,}원" if _c > 0 else "❌ 0원"
                    except Exception as _e:
                        _row["Naver"] = f"❌ {type(_e).__name__}"
                    _row["Naver(초)"] = f"{_t_mod.time()-_s:.1f}"

                    # ② yfinance
                    _s = _t_mod.time()
                    _row["yfinance"] = "❌ 실패"
                    if HAS_YFINANCE:
                        _isd = (len(_tk) == 6 and _tk[:4].isdigit())
                        for _sfx in ([".KS", ".KQ"] if _isd else [""]):
                            try:
                                _hh = get_yf_cached_data(_tk + _sfx).get("history")
                                if _hh is not None and not _hh.empty:
                                    _row["yfinance"] = f"✅ {int(_hh['Close'].dropna().iloc[-1]):,}원 ({_sfx or '해외'})"
                                    break
                            except Exception:
                                continue
                    else:
                        _row["yfinance"] = "⚪ 미설치"
                    _row["yf(초)"] = f"{_t_mod.time()-_s:.1f}"
                    ...
                    if HAS_FDR and fdr:
                        try:
                            _df = fdr.DataReader(_tk, start=today_kst()-datetime.timedelta(days=10))
                            _row["FDR"] = (f"✅ {int(_df['Close'].dropna().iloc[-1]):,}원"
                                           if _df is not None and not _df.empty else "❌ 빈 응답")
                        except Exception as _e:
                            _row["FDR"] = f"❌ {type(_e).__name__}"
                    _row["FDR(초)"] = f"{_t_mod.time()-_s:.1f}"
                ...
                _all_fail = all("✅" not in f"{r.get('Naver','')}{r.get('yfinance','')}{r.get('FDR','')}"
                                for r in _rows)
                if _all_fail:
                    st.error(
                        "**3개 소스 전부 실패** — 서버 IP가 차단된 상태입니다.\n\n"
                        "· Naver·yfinance 가 Streamlit Cloud IP를 차단하면 발생합니다.\n"
                        "· 앱 코드 문제가 아니며, 시간이 지나면 자동 복구되는 경우가 많습니다.\n"
                        "· **Manage app → Reboot app** 으로 IP를 재할당해 보세요.")
                else:
                    st.success(
                        "**일부 소스는 정상** — 데이터 수집 경로는 살아 있습니다.\n\n"
                        "특정 소스만 ❌ 라면 나머지로 자동 폴백되므로 정상 동작합니다. "
                        "화면이 여전히 '미수집'이면 아래 [캐시 비우기]를 눌러주세요.")

                if st.button("🧹 주가 캐시 비우고 새로고침", key="btn_clearcache",
                             use_container_width=True):
                    get_stock_data.clear()
                    get_yf_cached_data.clear()
                    st.rerun()
...
with tab_ai:
    ...
    with i1: st.info("📰 뉴스·지수")
    with i2: st.info("🔍 전종목 스크리닝")
    with i3: st.info("📢 DART 공시")
    with i4: st.info("🤖 AI 해석")
    with i5: st.info("📝 추천 기록")

    # ═══════════════════════════════════════════════════════
    #  v11 ① 보유 종목 긴급 공시 경고 (AI 호출 없이 즉시)
    # ═══════════════════════════════════════════════════════
    _dart_key = st.session_state.get("dart_key","")
    if _dart_key and HAS_DART_MOD:
        _my = tuple(sorted(get_my_tickers()))
        if _my:
            _disc = fetch_my_disclosures(_dart_key, _my, days=3)
            _crit = DART.get_critical_alerts(_disc)
            if _crit:
                st.error(MPROMPT.build_critical_alert_text(_crit)
                         if HAS_MPROMPT else f"긴급 공시 {len(_crit)}건")

    # ═══════════════════════════════════════════════════════
    #  v11 ② 전종목 스크리닝 결과
    # ═══════════════════════════════════════════════════════
    with st.expander("🔍 전종목 정량 스크리닝 (Layer 0~2)", expanded=False):
        st.caption("전체 상장종목 → 하드컷 필터 → 4팩터 랭킹. AI는 이 결과만 해석합니다.")
        cA, cB = st.columns([1,2])
        with cA:
            _topn = st.number_input("상위 N종목", 5, 30, 15, key="scr_topn")
        with cB:
            st.caption(" ")
            if st.button("🔄 스크리닝 실행", use_container_width=True, key="btn_scr"):
                run_full_screening.clear()
                st.session_state.screen_res = run_full_screening(int(_topn))
                st.rerun()

        _sr = st.session_state.get("screen_res")
        if _sr is None:
            st.info("👆 [스크리닝 실행]을 누르면 전종목을 분석합니다. (1~3분 소요)")
        elif _sr.get("error"):
            st.warning(f"⚠️ 스크리닝 실패: {_sr['error']}")
            _es = _sr.get("stats", {})
            if _es:
                # [v11.3] 어느 단계에서 전멸했는지 보여준다
                _steps = [
                    ("전체 수집",   _es.get("input", 0)),
                    ("우선주 제외", -_es.get("drop_preferred", 0)),
                    ("스팩 제외",   -_es.get("drop_spac", 0)),
                    ("시총 미달",   -_es.get("drop_marcap", 0)),
                    ("거래대금 미달", -_es.get("drop_turnover", 0)),
                    ("PER 부적격", -_es.get("drop_per", 0)),
                    ("PBR 부적격", -_es.get("drop_pbr", 0)),
                    ("하드컷 통과", _es.get("output", 0)),
                ]
                st.caption("단계별 종목 수 — " + " | ".join(
                    f"{n} {v:+,}" if n not in ("전체 수집", "하드컷 통과")
                    else f"{n} {v:,}" for n, v in _steps))
                if _es.get("per_missing"):
                    st.caption("ℹ️ 유니버스 소스에 PER/PBR 이 없어 네이버에서 보강을 시도했습니다. "
                               "네이버 조회가 막히면 이 단계에서 실패할 수 있습니다.")
            st.caption("실패해도 기존 워치리스트 기반 분석은 정상 동작합니다.")
        else:
            _st = _sr.get("stats", {})
            st.success(f"✅ {_sr.get('source')} 기준 {_st.get('input',0):,}종목 "
                       f"→ 하드컷 {_st.get('output',0):,}종목 "
                       f"→ 상위 {len(_sr['top'])}종목")
            m1,m2,m3 = st.columns(3)
            m1.metric("모멘텀 수집", f"{_st.get('momentum_collected',0)}종목")
            m2.metric("수급 수집",   f"{_st.get('flow_collected',0)}종목")
            m3.metric("기준일",      _sr.get("as_of","-"))
            _disp = _sr["top"][["code","name","total_score","score_value",
                                "score_quality","score_momentum","score_flow",
                                "close","per","pbr","roe"]].copy()
            _disp.columns = ["코드","종목명","종합","가치","퀄리티","모멘텀",
                             "수급","현재가","PER","PBR","ROE"]
    ...
    #  v11 ③ 추천 성과 검증
    # ═══════════════════════════════════════════════════════
    with st.expander("📝 내 추천 기록 & 적중률", expanded=False):
        st.caption("AI 추천을 시점·가격과 함께 저장하고 1주/1개월/3개월 뒤 실제 성과를 측정합니다.")
        _sb = get_supabase()
        if not (_sb and HAS_TRACK and st.session_state.get("user")):
            st.info("Supabase 연결 후 사용 가능합니다.")
        else:
            _uid = st.session_state.user.id
            _rows = TRACK.load_recommendations(_sb, _uid)
            st.markdown(TRACK.build_performance_summary(_rows))
            if st.button("🔃 성과 갱신 (실제 주가 반영)", key="btn_eval",
                         use_container_width=True):
                with st.spinner("실제 주가 조회 중..."):
                    def _pf(tk):
                        c,_,_ = get_stock_data(tk); return c
                    _r = TRACK.evaluate_recommendations(_sb, _uid, _pf)
                st.success(f"✅ {_r['message']}")
                st.rerun()
            if _rows:
                import pandas as _pd
                _rd = _pd.DataFrame(_rows)[[
                    c for c in ["rec_date","ticker","name","rec_price",
                                "target_price","price_1w","price_1m","price_3m"]
                    if c in _pd.DataFrame(_rows).columns]]
    ...
    if st.button("🔄 최신 데이터 수집 & AI 포트폴리오 분석 실행", type="primary", use_container_width=True):
        if not selected_model:
            ...
            # ── v11: 전종목 스크리닝 (Layer 0~2) ──────────────
            progress.progress(50,text="🔍 전종목 스크리닝 중...")
            _scr = st.session_state.get("screen_res")
            if _scr is None or _scr.get("error"):
                with st.spinner("전종목 정량 스크리닝 (1~3분)..."):
                    _scr = run_full_screening(15)
                    st.session_state.screen_res = _scr
            if _scr and not _scr.get("error") and _scr.get("top") is not None:
                _cand_ctx = SCREENER.build_candidate_context(_scr["top"], _scr["as_of"])
                _score_map = {r["code"]: {
                    "total": r.get("total_score"), "value": r.get("score_value"),
                    "quality": r.get("score_quality"), "momentum": r.get("score_momentum"),
                    "flow": r.get("score_flow")}
                    for _, r in _scr["top"].iterrows()}
                st.caption(f"🔍 스크리닝: {_scr['stats'].get('input',0):,}종목 "
                           f"→ 상위 {len(_scr['top'])}종목 후보 확보")
            else:
                _cand_ctx = "(스크리닝 미실행 — 기존 워치리스트 기반 분석)"
                _score_map = {}

            # ── v11: DART 공시 (Layer 확장) ───────────────────
            progress.progress(55,text="📢 DART 공시 수집 중...")
            _disc_ctx = "(DART 미연동)"
            _dk2 = st.session_state.get("dart_key","")
            if _dk2 and HAS_DART_MOD:
                _myt = tuple(sorted(get_my_tickers()))
                _dlist = fetch_my_disclosures(_dk2, _myt, days=5)
                if _dlist:
                    _disc_ctx = DART.build_disclosure_context(_dlist)

            # ── 스크리닝·공시를 시장 컨텍스트에 병합 ──────────
            mc = (mc
                  + "\n\n[정량 스크리닝 후보 — 신규 추천은 이 목록에서만]\n" + _cand_ctx
                  + "\n\n[DART 전자공시]\n" + _disc_ctx)

            progress.progress(60,text="🤖 AI 분석 시작...")
            try:
                ...
                # ── v11: 추천 종목 자동 기록 (사후 검증용) ────
                if HAS_TRACK and st.session_state.get("user"):
                    try:
                        _recs = TRACK.parse_recommendations(report)
                        if _recs:
                            _sv = TRACK.save_recommendations(
                                get_supabase(), st.session_state.user.id,
                                _recs, _score_map)
                            if _sv["status"] == "ok":
                                st.caption(f"📝 추천 {_sv['saved']}건 기록됨 — "
                                           f"1주 후 성과가 자동 측정됩니다.")
                    except Exception:
                        pass
