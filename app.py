...
STAGE1_MAX_TOKENS       = 2500  # [v28] 4500→2500: Stage1 압축으로 한도 감소 (3섹션 핵심만)
STAGE2_MAX_TOKENS       = 8192  # 종목 분석 + 압축된 액션플랜
...
def call_gemini(api_key, model_name, prompt, max_tokens,
    ...
    while truncated and cont_count<max_cont:
        ...
        # [v28] Stage2 섹션 5-8을 단일 ## 5로 통합 압축 → 토큰 절약
        _stage2_sections = [
            "## 4. 📊 보유 종목 분석",
            "## 5. 💡 종합 액션 플랜",
        ]
...
⚠️ 주의: "이미 작성 완료" 목록은 섹션 헤더 기준이며, 해당 섹션 내부가 잘린 경우
중단 직전 내용부터 이어서 완성한 뒤 다음 섹션으로 넘어가세요.
...
위 내용이 끊긴 지점부터 즉시 한국어 본문만 이어서 작성하세요."""
        try:
            ...
            # CONTINUATION_MAX_TOKENS(5000) 고정 시 Stage2(8192) 본문 이어받기가
            # 또 잘려 max_cont를 모두 소진하던 문제 해결.
            ...
            for c_model in [used_model]+[m for m in candidates if m!=used_model]:
                try:
                    ...
                    cont_text,cont_trunc=_call_single(c_obj,cont_prompt,_cont_max,temperature)
...
⑨-c ⛔ 손절가 도달 시 처리 규칙 — 반드시 준수:
   ...
   [예외 — 보유/추가매수 유지 가능 조건] 아래 중 하나 이상 충족 시에만:
     ① 전체 시장의 일시적 충격(베타 효과)에 따른 동반 하락이며 종목 펀더멘털 유효
     ② 매출성장률·FCF·PEG 등 핵심 펀더멘털 지표가 여전히 양호
     ③ 손절가 자체가 종목의 정상 변동성을 충분히 반영 못해 하향 조정 필요
     ④ 단기 노이즈성 이벤트(루머, 단발성 뉴스)로 인한 일시 하락
   [필수] 위 예외 적용 시 반드시 **'손절가 도달 보충 의견'** 필드 작성.
     · 객관적 수치·근거 필수 (예: "FCF 12% 성장 유지, PEG 0.8로 저평가 구간")
     · "기다려보자" "조정 후 반등 기대" 등 추상적 표현 절대 금지
     · 손절가 하향 조정 시 새 손절가와 그 근거 명시
...
- **투자의견**: 강력매수 / 추가매수 / 보유 / 비중축소 / 매도
  → *변경 없음 OR "변경 사유: OOO 조건 충족"* | 매입가·지표·수급·펀더멘털·트렌드 종합 판단
- **현재가 / 매입가**: curr=OOO원 / 평단 OOO원 (수익률 ±OO%)
- **목표가 / 손절**: OOO원(×O.O배, PER기반) / 평단 OOO원×0.XX=OOO원(손절폭 OO%, 근거: OO)
  → 손절 기준가: 손실 중→평단 기준 / 수익 중→현재가 기준 | 손절폭은 베타·RSI·수급 등 가변
- **⚠️ 손절가 도달 보충 의견** *(조건부 — 현재가 ≤ 손절가 인데 의견이 매도/비중축소가 아닌 경우 반드시 작성)*
  → 예시 형식: "현재가 OOO원 ≤ 손절가 OOO원 도달. 그럼에도 [보유/추가매수] 유지 사유:
     [객관적 근거 — FCF·PEG·매출성장 등 구체 수치]. 손절가 OOO원으로 하향 조정 권고(근거: OO)."
  → 해당 안 되는 종목(현재가 > 손절가)은 이 필드 자체를 출력하지 말 것
- **핵심 지표**: [개별주] PER·PEG·매출성장·FCF | [ETF] 베타·분배율·총보수·지수방향 | RSI OO(OO신호) | MA60 ±OO%
- **수급**: 외국인·기관 순매수 방향 (미수집 시 RSI·MA 대체)
- **투자 근거**: 핵심 성장 스토리 + 메가트렌드 연결 1~2줄
- **절세**: 계좌별 절세 포인트 1줄

---
## 5. 💡 종합 액션 플랜 (요약 — 모든 부가 정보는 핵심만)

> ⛔ 아래 4개 소항목은 각각 **3~5줄 이내**로 압축. 길게 풀어쓰지 말 것.
> 보유 종목 분석(## 4)에 우선 토큰 할당하기 위함.

### 💎 신규 추천 (10루타 후보)
- 최대 1~2종목만. 종목당 1줄: **종목명 (코드) | 메가트렌드 연결 | 10루타 조건 ①②③ 중 충족 / 현재가·3년 목표가 / 분할매수 전략 1줄**

### ⚖️ 포트폴리오 리밸런싱 (3~6개월 우선 액션)
- 핵심 액션 2~3개만 1줄씩 (예: "AI 비중 ↑ 5%p / 채권 ETF ↓ 3%p / 신규 추천종목 진입")

### 📅 모니터링 지표 (펀더멘털 훼손 조기 경보)
- 3개 지표를 한 줄씩: **지표명 | 현재값 | 경보 임계값**

### 📝 최종 결론
- **한줄 요약**: 현재 포트폴리오의 핵심 리스크·기회 (1문장)
...
"""
    ...
    def _clean_report(report: str) -> str:
        ...
        # ── 0차: AI 영어 추론 출력 줄 제거 ────────────────────────────────────
        # [C-2 수정] 정규식을 명백한 AI 자기참조 추론만 잡도록 대폭 축소.
        # 기존의 'Note:', 'This is', 'Usually,' 패턴은 정상 영문 본문
        # (예: ETF 영문 설명)을 오삭제할 위험이 있어 제거.
        # 남긴 패턴은 한국어 투자 리포트 본문에 정상적으로 등장할 수 없는
        # 명백한 AI 작업 메타발언만 매칭.
        clean_lines = []
        reasoning_patterns = [
            r'^\s*I (will|would|am going to) (continue|now|start|finish|proceed|re)',
            r'^\s*I\'ll (continue|now|start|finish|proceed)',
            r'^\s*\*+\s*\*?Wait\*',                 # "*   *Wait*" AI 망설임
            r'^\s*Let me (continue|finish|re-?read|check|see|proceed)',
            r'^\s*\(continuing from',
        ]
        # 마크다운 구조 줄(헤더·불릿·인용·표)은 절대 삭제 대상 아님 — 추가 안전장치
        _md_struct = ('#', '-', '>', '|', '─', '━', '✅', '⛔', '※')
        for line in report.split('\n'):
            ...
            if not is_struct and any(_re.match(p, line) for p in reasoning_patterns):
                continue  # 명백한 AI 추론 줄만 제거
            clean_lines.append(line)
        report = '\n'.join(clean_lines)

        # ── 1차: ✅ 분석 완료: 이후 고아 내용 제거 ────────────────────────────
        # [v28] 마지막 섹션이 ## 5로 변경됨 (4-7 → 4만 검사)
        comp_marker = "✅ 분석 완료:"
        ...
        if ci > 200:
            ...
            if eol > ci:
                ...
                if tail:
                    # ## 4 재등장(반복 버그) 또는 ## 5/---가 아닌 고아 내용 → 제거
