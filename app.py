...
"""
    ...
    def _clean_report(report: str) -> str:
        ...
        # ── 0차: AI 영어 추론 출력 줄 제거 ────────────────────────────────────
        # "I will...", "*Wait*", "Let me..." 등 AI 사고과정이 본문에 섞인 경우
        clean_lines = []
        reasoning_patterns = [
            r'^\s*I (will|would|should|can|think|need|notice)',
            r'^\s*\*\s*\*?Wait\*?\*?',
            r'^\s*Let me ',
            r'^\s*This is (slightly|a bit|somewhat)',
            r'^\s*Usually,',
            r'^\s*Note:?\s+(the|this|that|I)',
        ]
        for line in report.split('\n'):
            if any(_re.match(p, line) for p in reasoning_patterns):
                continue  # AI 추론 줄 제거
