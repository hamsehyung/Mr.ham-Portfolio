"""
=============================================================
  📈 Mr.Ham — 24Hr AI 포트폴리오 매니저 v30.0 (국내상장주식용)
=============================================================
  🔐 보안 정책
...
STAGE1_MAX_TOKENS       = 2500  # [v29] 4500→2500: Stage1 압축으로 한도 감소
...
⑫ ⛔ 포트폴리오 진단(## 5) 작성 의무 — AI 포트폴리오 분석가로서의 핵심 역할:
   · [포트폴리오 자산배분] 데이터와 [보유 포트폴리오]의 종목별 평가금액을 직접 합산해 비중 계산.
   · 단일 종목 30% 초과 시 반드시 "분산 권고", 50% 초과 시 "위험" 평가.
   · 섹터 노출은 보유 종목의 메가트렌드 정렬을 기반으로 분류 (예: 반도체·AI·채권 ETF·미국 지수 등).
   · 해외 ETF 비중을 환율 노출로 간주하여 USD 변동 영향도 1줄 평가.
   · 계좌별 분포를 절세 관점에서 1줄 평가 (예: "ISA 비중 부족 — 비과세 혜택 활용도 낮음").
...
- **펀더멘털**: PER OO·PEG OO·매출성장±OO%·FCF OO억·부채OO% | 시총 OO조·배당률OO%
- **시세 위치**: 52주위치 OO%(고점대비-OO%) | RSI OO(신호) | MA60 ±OO%
- **수급**: 외국인/기관 방향 (미수집→"N/A, RSI/MA 대체")
...
### 🔍 포트폴리오 진단 (전체 현황 — 분산·집중도·편중 평가)
- **종목 집중도**: 상위 1종목 OO% / 상위 3종목 OO% — 평가: [양호 / 주의(30%+) / 위험(50%+)]
- **자산 클래스**: 개별주 OO% / 국내ETF OO% / 해외ETF OO% — 변동성 균형 평가 1줄
- **섹터 노출**: 추출된 메가트렌드별 분포 (예: 반도체 OO% / 채권 ETF OO% / 미국 지수 OO% / 기타 OO%)
- **환율 노출**: 해외 자산 비중 OO% (원/달러 변동 영향도 1줄 평가)
- **계좌별 분포**: 일반 OO% / ISA OO% / 퇴직연금 OO% / 연금저축 OO% — 절세 효율 1줄
- **핵심 리스크**: 위 진단에서 도출된 가장 큰 리스크 1줄 (예: "반도체 집중 65% — 산업 사이클 충격에 취약")

### 💎 신규 추천 (10루타 후보)
- 최대 1~2종목. 종목당 1줄: **종목명(코드) | 메가트렌드 | 10루타 조건 ①②③ 중 충족 | 현재가/목표가 | 분할매수 1줄**

### ⚖️ 포트폴리오 리밸런싱 (3~6개월)
- 핵심 액션 2~3개를 1줄씩 (예: "AI 비중 +5%p / 채권 ETF -3%p")

### 📅 모니터링 지표 (펀더멘털 훼손 조기 경보)
- 3개 지표를 1줄씩: **지표명 | 현재값 | 경보 임계값**
...
if _ICON_URI:
    st.markdown(f"""<h1 style='display:flex;align-items:center;gap:10px;
        ...
        Mr.Ham &nbsp;|&nbsp; 24Hr AI 포트폴리오 매니저 v30.0
        </h1>""", unsafe_allow_html=True)
else:
    st.title("📈 Mr.Ham  |  24Hr AI 포트폴리오 매니저 v30.0")
