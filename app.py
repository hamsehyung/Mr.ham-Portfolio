...
def _call_single(model_obj, prompt, max_tokens, temperature=0.0):
    # [수정] temperature를 파라미터로 받음
    # Stage1(매크로 브리핑): 0.1 — 매번 약간의 다양성 허용
    # Stage2(종목 분석):     0.0 — 결정론적 출력, 동일 입력 → 동일 의견
...
def call_gemini(api_key, model_name, prompt, max_tokens,
                ...
                temperature=0.0):   # [수정] temperature 파라미터 추가
    genai.configure(api_key=api_key)
    ...
    for attempt,model in enumerate(candidates):
        ...
        try:
            ...
            full_text,truncated=_call_single(m_obj,prompt,max_tokens,temperature)  # [수정] temperature 전달
        ...
        try:
            for c_model in [used_model]+[m for m in candidates if m!=used_model]:
                try:
                    ...
                    cont_text,cont_trunc=_call_single(c_obj,cont_prompt,CONTINUATION_MAX_TOKENS,temperature)  # [수정]
...
원칙 4. 10루타(10-bagger) 잣대 — 신규 추천 필수 조건 3개 이상 충족
  ...
  ② PEG < 1.5 또는 매출성장률 > 25% (하이그로스 인정)
  ③ FCF 양수 또는 2년 내 흑자 전환 가시성 존재
  ④ 글로벌 경쟁 우위 (특허·브랜드·네트워크 효과 중 하나)
  ⑤ 현재 저평가 or 시총 5조원 이하의 숨겨진 우량 기업
...
- **투자의견**: 강력매수 / 추가매수 / 보유 / 비중축소 / 매도
  → *(변경 없음 또는 "변경 사유: OOO 조건 충족" 중 하나를 반드시 명시)*
- **장기 목표가**: OOO원 *(3년 기준, 근거: Forward PER OO배 × 예상 EPS)*
- **미래 가치 분석**:
  - [개별주] ForwardPER | PEG | 매출성장 | FCF | 부채비율 수치 분석
  - [ETF] 베타 | 분배율 | 총보수 | 기초지수 방향성
- **메가트렌드 정렬**: 해당 종목이 수혜를 받는 장기 트렌드 명시
- **이동평균 위치**: MA60 대비 ±OO% *(단순 참고, 매매 신호로 사용 금지)*
- **수급 동향**: 외국인▲/▼ | 기관▲/▼ *(장기 추세 방향만 참고)*
- **핵심 투자 근거**: 비즈니스 모델과 장기 성장 스토리 중심 2줄
- **손절 기준**: OOO원 *(펀더멘털 훼손 시점 기준)*
- **계좌 절세 포인트**: 1줄
...
"""
    # [수정] Stage2 temperature=0.0 (결정론적, 동일 입력 → 동일 의견 보장)
