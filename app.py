...
⑨-b ⛔ 손절 기준 — 일률 고정 비율 절대 금지. 아래 2단계로 종목별 가변 산출:

  [1단계] 기준가 선택 — 수익/손실 상황에 따라 다름
   · 손실 중 (현재가 < 평단): 평균매입가(avg) 기준 → 원금 보호 관점. 평단보다 높은 손절선 유지.
   · 수익 중 (현재가 > 평단): 현재가(curr) 기준 → 수익 보존(트레일링 스탑) 관점.

  [2단계] 허용 손실률 가변 산출 (기본 -10%, 아래 조건으로 조정)
   완화 요인 (손절폭 넓게):
    · 고베타(>1.3) 또는 반도체·AI 등 고변동성 성장 업종 → +3~5%p
    · PEG<1 또는 FCF 양호하고 구조적 성장 메가트렌드 수혜 → +2~3%p
    · RSI<35 과매도 구간 (기술적 반등 여지) → +2%p
    · 외국인·기관 순매수 지속 중 → +1~2%p
   강화 요인 (손절폭 좁게):
    · 적자 기업 또는 부채비율 과다 → -2~3%p
    · 구조적 쇠퇴 업종 또는 메가트렌드 역방향 → -2~3%p
    · RSI>70 이후 하락 추세 전환 → -2%p
   최종 허용 범위: 최소 -7% ~ 최대 -20% (이 범위 밖 산출 금지)
...
- **투자의견**: 강력매수 / 추가매수 / 보유 / 비중축소 / 매도
  ...
  → 투자의견은 아래 모든 항목(매입가·기술지표·수급·펀더멘털·메가트렌드)을 종합한 판단
- **실시간 현재가**: OOO원 ← [참고 시세]의 curr= 값을 그대로 옮겨 적을 것
- **매입가 현황**: 평단 OOO원 → 현재 OOO원 (수익률 ±OO%)
  → [참고 시세]의 avg=, curr= 기반으로 작성. 수익률은 참고 맥락이며 기계적 매매 기준 아님.
  → 수익률 수치와 무관하게 펀더멘털·장기 성장성·밸류에이션이 주 판단 근거
- **장기 목표가**: (위 현재가) × O.O배 = OOO원 / 근거: ForwardPER·PEG 기반
- **손절 기준**: [기준가 OOO원] × [0.XX] = OOO원
  → 기준가: 손실 중→평단(원금 보호) / 수익 중→현재가(트레일링)
  → 손절폭 [OO%] 근거: [핵심 결정 요인 1줄 — 베타·RSI·PEG·수급·트렌드 중 반영 요소 명시]
- **미래 가치 분석**:
...
"""
    ...
    def _clean_report(report: str) -> str:
        """
        프롬프트 지시사항 노출 제거 + 이어받기 오염 패턴(section 반복) 제거.
        0차: ✅ 분석 완료: 이후 ## 4.~## 7. 섹션 재등장 차단 (핵심 버그 픽스)
        1차: ⛔ [리포트 종료 선언] 마커 제거
        2차: 보조 마커 줄 선두 제거
        """
        import re as _re

        # ── 0차: ✅ 분석 완료: 이후 고아 내용 제거 ────────────────────────────
        # 이어받기(continuation) 중 AI가 extra_context를 출력하거나
        # ## 4. 섹션을 처음부터 다시 생성하는 오염 패턴 차단
        comp_idx = report.find("✅ 분석 완료:")
        if comp_idx > 200:
            eol = report.find("\n", comp_idx)
            if eol > comp_idx:
                tail = report[eol:].strip()
                if tail:
                    # section 4~7 재등장 (반복 버그) 또는 비섹션 고아 내용 → 제거
                    if (_re.match(r'^##\s*[4-7][\.\s]', tail)
                            or (not tail.startswith("## 8") and not tail.startswith("---"))):
