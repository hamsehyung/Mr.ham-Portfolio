"""
=============================================================
  📈 Mr.Ham — 24Hr AI 포트폴리오 매니저 v14.0 (국내상장주식용)
=============================================================
  🔐 보안 정책
...
STAGE1_MAX_TOKENS       = 4500  # [Fix1] 3000→4500: Stage1 잘림 빈도 감소
STAGE2_MAX_TOKENS       = 8192
CONTINUATION_MAX_TOKENS = 5000  # [Fix2] 4000→5000: 이어받기 품질 향상

# ── 한국 표준시 (KST = UTC+9) 상수 — 날짜·시간 전역 사용 ──
KST = datetime.timezone(datetime.timedelta(hours=9))

# ── 공용 아이콘 SVG (3×3 바둑판 배경 + 빨간 상승선) ──────────────────────
# 참고 이미지와 동일한 구조, 색상만 파란→빨간으로 교체
_MRHAM_ICON_SVG = """<svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
  <rect width="100" height="100" rx="22" fill="white" stroke="#ede9f8" stroke-width="1.5"/>
  <rect x="6"  y="6"  width="27" height="27" rx="5" fill="#dddaf0"/>
  <rect x="37" y="6"  width="27" height="27" rx="5" fill="#dddaf0"/>
  <rect x="67" y="6"  width="27" height="27" rx="5" fill="#dddaf0"/>
  <rect x="6"  y="36" width="27" height="27" rx="5" fill="#dddaf0"/>
  <rect x="37" y="36" width="27" height="27" rx="5" fill="#dddaf0"/>
  <rect x="67" y="36" width="27" height="27" rx="5" fill="#dddaf0"/>
  <rect x="6"  y="67" width="27" height="27" rx="5" fill="#dddaf0"/>
  <rect x="37" y="67" width="27" height="27" rx="5" fill="#dddaf0"/>
  <rect x="67" y="67" width="27" height="27" rx="5" fill="#dddaf0"/>
  <polyline points="10,84 30,60 50,72 72,30 90,16"
            stroke="#e03030" stroke-width="6" fill="none"
...
_FAVICON_SVG_URI = (
    ...
    "<rect width='100' height='100' rx='22' fill='white' stroke='%23ede9f8' stroke-width='1.5'/>"
    "<rect x='6'  y='6'  width='27' height='27' rx='5' fill='%23dddaf0'/>"
    "<rect x='37' y='6'  width='27' height='27' rx='5' fill='%23dddaf0'/>"
    "<rect x='67' y='6'  width='27' height='27' rx='5' fill='%23dddaf0'/>"
    "<rect x='6'  y='36' width='27' height='27' rx='5' fill='%23dddaf0'/>"
    "<rect x='37' y='36' width='27' height='27' rx='5' fill='%23dddaf0'/>"
    "<rect x='67' y='36' width='27' height='27' rx='5' fill='%23dddaf0'/>"
    "<rect x='6'  y='67' width='27' height='27' rx='5' fill='%23dddaf0'/>"
    "<rect x='37' y='67' width='27' height='27' rx='5' fill='%23dddaf0'/>"
    "<rect x='67' y='67' width='27' height='27' rx='5' fill='%23dddaf0'/>"
    "<polyline points='10,84 30,60 50,72 72,30 90,16'"
    " stroke='%23e03030' stroke-width='6' fill='none'"
    " stroke-linecap='round' stroke-linejoin='round'/>"
    "</svg>"
)
...
def fetch_market_indices() -> dict:
    ...
    for name, ticker in MARKET_TICKERS.items():
        try:
            ...
            hist = hist.dropna(subset=["Close"])   # NaN 방어 — NaN 포함 시 round() 오류 방지
            if len(hist) >= 2:
                ...
                result[name] = {"current": round(float(hist["Close"].iloc[-1]),2), "change": 0, "pct": 0}
        except: continue
    return result
...
def fetch_investor_trend_raw(ticker: str) -> dict:
    ...
    for ep in endpoints:
        try:
            r = _NAVER_INV_SESSION.get(ep, timeout=8)
...
<div style="background:#f8f9fa;padding:14px 16px;border-radius:10px;
  ...
  <div style="font-size:14px;line-height:1.9">
    평균단가: {avg_price:,}원 &nbsp;→&nbsp; <b>현재가: {curr:,}원</b><br>
...
def _warmup_portfolio_cache(portfolio: dict) -> None:
    ...
    # 최대 8 스레드 — Streamlit Cloud 환경 기준 안전한 병렬 수
    with ThreadPoolExecutor(max_workers=min(8, len(tickers))) as ex:
        futures = [ex.submit(_fetch_one, tk) for tk in tickers]
        for f in as_completed(futures):
            pass  # 결과 무시, 캐시 사이드이펙트만 활용
...
def fetch_watchlist_prices(watchlist_tuple: tuple) -> dict:
    ...
    with ThreadPoolExecutor(max_workers=max(1, min(8, len(watchlist_tuple)))) as ex:
        for tk, result in ex.map(_fetch_one, watchlist_tuple):
            if result:
                r[tk] = result
    return r
...
def call_gemini(api_key, model_name, prompt, max_tokens,
    ...
    while truncated and cont_count<max_cont:
        ...
        # ── [Fix3] Stage별 허용 섹션 분리: Stage1은 ##1~##3, Stage2는 ##4~##8
        _stage2_sections = [
            "## 4. 📊 보유 종목 분석",
            "## 5. 💡 신규 추천 종목 (10루타 후보)",
            "## 6. ⚖️ 포트폴리오 리밸런싱 제언",
        ...
        _last_section = "## 3." if _is_stage1 else "## 8."
        _section_range = "## 1 ~ ## 3" if _is_stage1 else "## 4 ~ ## 8"
        ...
        _extra_block = f"\n[포트폴리오 참고 데이터 — 반드시 이 데이터의 종목만 분석할 것]\n{extra_context}" if extra_context else ""
...
[중단 직전 내용 — 여기서 바로 이어서 작성]
──────────────────────────────────────
{full_text[-1500:].strip()}
──────────────────────────────────────
끊긴 부분부터 이어서 [남은 섹션]만 완성하세요."""
        try:
...
⑥-b ⛔ 매입가 현황 필드 작성 필수 — [참고 시세]의 avg= 와 curr= 를 반드시 기입.
   수익률 = (curr-avg)/avg×100. 단, 수익률 숫자만으로 매수·매도를 기계적으로 결정하지 말 것.
   수익률은 참고 맥락일 뿐이며, 투자의견은 반드시 기업 펀더멘털·장기 성장성·밸류에이션을 주된 기준으로 판단.
   (예: +50% 수익 중이라도 성장 스토리가 유효하면 "보유·추가매수" 가능.
        -20% 손실 중이라도 일시적 조정이면 "보유·분할매수" 가능.
        반대로 +10% 수익이라도 펀더멘털 훼손 시 "비중축소" 판단 가능.)
...
⑩ ⛔ 섹션 범위 엄수 — 이 리포트는 ## 4 ~ ## 8 이 전부. ## 9 이후 섹션 생성 절대 금지.
...
- **실시간 현재가**: OOO원 ← [참고 시세]의 curr= 값을 그대로 옮겨 적을 것
- **매입가 현황**: 평단 OOO원 → 현재 OOO원 (수익률 ±OO%)
  → 수익률은 참고 맥락으로만 사용. 투자의견은 펀더멘털·성장성·밸류에이션 기준으로 판단.
- **장기 목표가**: (위 현재가) × O.O배 = OOO원 / 근거: ForwardPER·PEG 기반
- **손절 기준**: (위 현재가) × 0.XX = OOO원 ← 현재가 대비 비율로만 산출
- **미래 가치 분석**:
...
- **장기 투자자를 위한 최종 한줄 요약**: 현재 포트폴리오의 핵심 리스크와 기회를 압축한 단 한 문장
- **향후 리밸런싱 시점 제언**: 다음 리밸런싱을 실행해야 할 구체적 조건 또는 시점 (예: "KOSPI 2,400p 이탈 시", "Fed 금리 인하 2회 확인 후" 등)

✅ 분석 완료: OO개 / OO개 (자기 검증)
"""
    # [Fix5c] Stage2 호출 — 전용 섹션 목록 + 포트폴리오 요약 이어받기 주입
    ...
    s2=call_gemini(api_key, model_name, stage2, STAGE2_MAX_TOKENS,
                   ...
                   extra_context=_pf_summary)  # 이어받기 시 포트폴리오 데이터 재주입 → 가상 종목 방지
    ...
    def _clean_report(report: str) -> str:
        """
        [M3 수정] 프롬프트 지시사항이 응답에 노출된 경우 자동 제거.
        1차 마커(종료 선언)로 먼저 잘라내고, 없을 때만 보조 마커 탐색.
        보조 마커는 줄 선두에서만 매칭해 리포트 본문 중간 오탐 방지.
        [Fix2] ✅ 분석 완료: 이후 고아 내용(이어받기 중 extra_context 오염) 제거.
        """
        # 0차: ✅ 분석 완료: 줄 이후 고아 내용 제거 (이어받기 오염 패턴)
        # "✅ 분석 완료: 14개 / 14개" 한 줄은 유지하되, 그 줄 이후 불필요 내용 삭제
        comp_marker = "✅ 분석 완료:"
        ...
        if ci > 200:
            ...
            if eol > ci:
                ...
                # 완료 줄 바로 뒤에 섹션 헤더(##)가 아닌 내용이 오면 고아로 판단 → 제거
        ...
        # 2차: 보조 마커 — 줄 선두에 있을 때만 잘라냄 (본문 내 우연한 등장 방지)
        secondary_lines = [
            "이 리포트는 ## 4 ~ ## 8 이 전부입니다.",
            "## 9, ## 10 이후 섹션을 절대 생성하지 마십시오.",
        ]
        for marker in secondary_lines:
            idx = report.find("\n" + marker)
            if idx > 200:
                return report[:idx].rstrip()
...
@media (max-width: 768px) {
    ...
    /* selectbox 드롭다운 팝업 — 모바일에서 충분한 높이 보장 */
    [data-baseweb="popover"] {
...
st.markdown(f"""
<div style='display:flex;align-items:center;gap:10px;padding:0.6rem 0 0.2rem;'>
    <div style='flex-shrink:0;width:52px;height:52px;'>{_MRHAM_ICON_SVG}</div>
    <h1 style='margin:0;padding:0;font-size:1.875rem;font-weight:700;line-height:1.2;'>
        Mr.Ham &nbsp;&nbsp;|&nbsp;&nbsp; 24Hr AI 포트폴리오 매니저 v14.0
    </h1>
</div>
""", unsafe_allow_html=True)
...
(function setRedFavicon(){{
    ...
    try {{ applyFavicon(); }} catch(e){{}}
    setTimeout(applyFavicon, 300);
    setTimeout(applyFavicon, 1000);
    setTimeout(applyFavicon, 3000);
}})();
</script>
""", height=0)
