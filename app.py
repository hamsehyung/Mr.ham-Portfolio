...
def build_portfolio_text(portfolio: dict) -> str:
    ...
    for tk,m in merged.items():
        ...
        if is_etf(m["name"]):
            ...
            # 적정가: Forward PER × EPS (미래 지향적), 없으면 N/A
            per_tgt = ""
            if fund.get("forward_per") and fund.get("eps") and fund["forward_per"] > 0:
                per_tgt = f"ForwardPER기반적정가:{int(fund['forward_per'] * fund['eps']):,}원"
            elif fund.get("per") and fund.get("eps") and fund["per"] > 0:
                per_tgt = f"TrailingPER기반참고가:{int(fund['per'] * fund['eps']):,}원(참고용)"
