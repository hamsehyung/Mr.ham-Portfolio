...
def _naver_price(ticker):
    ...
    # 2. [가장 강력한 우회책] 네이버 금융 PC웹 HTML 직접 크롤링 (해외 IP API 차단 무력화)
    try:
        r = _http.get(f"https://finance.naver.com/item/main.naver?code={ticker}",
                      headers={"User-Agent": "Mozilla/5.0"}, timeout=3)
        if r.status_code == 200:
            html = r.text
            curr = high = low = 0

            # 정규식을 통한 현재가 추출 (가장 안정적인 접근성 태그 라인 활용)
            curr_match = re.search(r'<dd>현재가\s*([\d,]+)\s*전일대비', html)
            if not curr_match:
                curr_match = re.search(r'class="no_today".*?class="blind">([\d,]+)</span>', html, re.DOTALL)
            if curr_match:
                curr = ti(curr_match.group(1))

            # 정규식을 통한 52주 최고/최저가 추출
            hw_match = re.search(r'52주최고.*?<em[^>]*>([\d,]+)</em>.*?<em[^>]*>([\d,]+)</em>', html, re.DOTALL)
            if hw_match:
                high = ti(hw_match.group(1))
                low  = ti(hw_match.group(2))
