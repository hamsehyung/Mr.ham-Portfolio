...
with st.sidebar:
    ...
    with st.expander("📢 DART 공시 연동 (선택)", expanded=False):
        ...
        _dk = st.text_input(
            ...
            value=st.session_state.get("dart_key",""), key="dart_key_input",
            help="opendart.fss.or.kr 에서 무료 발급 (일 20,000건)")
        if _dk != st.session_state.get("dart_key",""):
            st.session_state.dart_key = _dk
        if st.session_state.get("dart_key"):
            st.success("✅ DART 연동됨")
        else:
