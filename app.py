...
🚨 [최우선 규칙]: 전달받은 [보유 포트폴리오]에 나열된 **모든 종목(총 몇 개이든 상관없이)을 단 하나도 빠짐없이 100% 개별 분석**할 것. 
일부 종목만 분석하고 "나머지 종목은..." 식으로 임의 생략하거나 요약하면 치명적인 시스템 오류로 간주함. 
일반 규칙: ①보유목록 외 추가금지 ②동일종목 복수계좌→합산1건 ③신규추천=미보유만 ④데이터없으면N/A

## 4. 📊 보유 종목 분석
(경고: 전달받은 포트폴리오 종목 개수와 아래 분석되는 종목의 개수가 정확히 일치해야 합니다.)
...
## 5. 💡 신규 추천 종목
### 📌 [[계좌]] 추천: [종목명] ([코드])
- 현재가·목표가·추천근거·분할매수·손절가
---
## 6. ⚖️ 리밸런싱 제언
- 현재 vs 적정 배분 / 액션 플랜 3단계
---
## 7. 📅 모니터링 포인트
- 주요 이벤트 3개 (날짜·지표·임계값)
"""
...
<style>
/* ════════════════════════════════════════
   헤더·푸터 숨기기 (토글 버튼은 예외 처리)
════════════════════════════════════════ */
header { visibility: hidden; }
/* ★ 핵심: header가 hidden이어도 토글 버튼은 반드시 표시 ★ */
header [data-testid="collapsedControl"],
header [data-testid="stSidebarCollapseButton"] { visibility: visible !important; }
#MainMenu { visibility: hidden; }
footer { visibility: hidden; }

/* ════════════════════════════════════════
   ★★ 사이드바 재열기 버튼 (접혔을 때 나타남) ★★
   · 주황색 — 라이트·다크모드 모두 선명하게 보임
   · 화면 왼쪽 중앙 고정 (position: fixed)
   · pulse 애니메이션으로 존재감 강조
   · z-index: 999999 — 모든 레이어 위에 표시
════════════════════════════════════════ */
[data-testid="collapsedControl"] {
    ...
    background:      #ff6b2b  !important;  /* 주황색 — 다크·라이트 모두 선명 */
    border-radius:   0 16px 16px 0 !important;
    border:          2.5px solid rgba(255,255,255,0.5) !important;
    box-shadow:      4px 0 20px rgba(255,107,43,0.75),
                     0   0 0   1px rgba(0,0,0,0.1)    !important;
    z-index:         999999   !important;
    cursor:          pointer  !important;
    animation:       sb-pulse 2.8s ease-in-out infinite !important;
}
[data-testid="collapsedControl"]:hover {
    background:  #e85a1e !important;
    box-shadow:  6px 0 28px rgba(255,107,43,1.0) !important;
...
[data-testid="collapsedControl"] span {
    ...
    display: none  !important;   /* 텍스트 숨기고 아이콘만 */
}
@keyframes sb-pulse {
    0%,100% { box-shadow: 4px 0 20px rgba(255,107,43,.75), 0 0 0 1px rgba(0,0,0,.1); }
    50%     { box-shadow: 4px 0 32px rgba(255,107,43,1.0), 0 0 0 1px rgba(0,0,0,.1); }
}

/* ════════════════════════════════════════
   ★★ 사이드바 닫기 버튼 (펼쳐졌을 때) ★★
   · 파란 원형 버튼 — 어디서든 눈에 잘 띔
   · 다크모드에서도 흰 아이콘으로 명확히 표시
════════════════════════════════════════ */
[data-testid="stSidebarCollapseButton"] > button,
[data-testid="stSidebar"] button[kind="header"],
[data-testid="stSidebar"] .stButton > button[aria-label*="sidebar"],
[data-testid="stSidebar"] .stButton > button[aria-label*="Sidebar"] {
    visibility:      visible  !important;
    ...
    align-items:     center   !important;
    justify-content: center   !important;
    box-shadow: 0 2px 10px rgba(28,131,225,0.55) !important;
...
/* 다크모드 — 재열기 버튼 테두리 더 강조 */
@media (prefers-color-scheme: dark) {
    [data-testid="collapsedControl"] {
        border: 2.5px solid rgba(255,255,255,0.8) !important;
        box-shadow: 4px 0 24px rgba(255,107,43,0.9),
