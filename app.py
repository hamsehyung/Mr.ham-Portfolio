...
try:
    import mrham_screener as SCREENER
    HAS_SCREENER = True
except Exception:
    HAS_SCREENER = False; SCREENER = None
try:
    import mrham_dart as DART
    HAS_DART_MOD = True
except Exception:
    HAS_DART_MOD = False; DART = None
try:
    import mrham_track as TRACK
    HAS_TRACK = True
except Exception:
    HAS_TRACK = False; TRACK = None
try:
    import mrham_prompt as MPROMPT
    HAS_MPROMPT = True
except Exception:
    HAS_MPROMPT = False; MPROMPT = None
        ...
        if _dk_in and _dv is None:
            if st.button("🔑 키 검증하기", key="btn_dart_verify",
                ...
                if not HAS_DART_MOD:
                    st.session_state.dart_validated = {
                        ...
                        "message": "mrham_dart.py 모듈이 없습니다."}
                else:
            ...
            if _dv["valid"]:
                ...
                _icon = {"bad_format": "✏️", "invalid_key": "🔑",
                         "no_permission": "🚫", "network": "📡"}.get(_dv["status"], "❌")
