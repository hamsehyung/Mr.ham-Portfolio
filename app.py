...
def get_yf_cached_data(ticker: str) -> dict:
    ...
    if not HAS_YFINANCE: return result
    try:
        # [v31 타임아웃] yfinance 무응답 시 영원히 대기하는 문제 수정
        # ThreadPoolExecutor + future.result(timeout) 으로 최대 8초 제한
        ...
        with _cf.ThreadPoolExecutor(max_workers=1) as _ex:
            ...
            try:
                _info, _hist = _fut.result(timeout=8)
                result["info"] = _info
                result["history"] = _hist
            except _cf.TimeoutError:
                pass  # 8초 초과 → 빈 결과 반환 (앱 블로킹 방지)
    except:
        pass
    return result
...
def _warmup_portfolio_cache(portfolio: dict) -> None:
    ...
    # [v31 타임아웃] 전체 예열 최대 25초 — 초과 시 미완료 스레드 취소하고 진행
    # 기존: as_completed(futures) → 무한 대기 가능 → 포트폴리오 탭 프리즈 버그
    from concurrent.futures import wait as _wait
    with ThreadPoolExecutor(max_workers=min(8, len(tickers))) as ex:
        futures = [ex.submit(_fetch_one, tk) for tk in tickers]
        done, pending = _wait(futures, timeout=25)
        for f in pending:
            f.cancel()  # 25초 내 미완료 → 취소 (블로킹 해제)
