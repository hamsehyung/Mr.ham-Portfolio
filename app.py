"""
=============================================================
  📈 Mr.Ham — 24Hr AI 포트폴리오 매니저 v10.0_국내상장용
=============================================================
  🔐 보안 정책
...
def save_last_report(report: str, date_str: str) -> None:
    ...
    idx = report.find("## 4.")
    # Gemini 1.5 이상의 거대한 컨텍스트 창을 100% 활용하기 위해 3000자 제한을 해제합니다.
    opinions = report[idx:] if idx >= 0 else report
...
def fetch_naver_fundamentals(ticker: str) -> dict:
    ...
    # 영문 혼용 6자리 국내 티커 완벽 대응
    if not (len(ticker) == 6 and ticker[:4].isdigit()):
        if HAS_YFINANCE:
            try:
...
def fetch_etf_naver_data(ticker: str) -> dict:
    result: dict = {}
    if not ticker or not (len(ticker) == 6 and ticker[:4].isdigit()): return result
    SKIP = {"","-","--","n/a","해당없음","해당 없음"}
    def sf(raw):
        c = re.sub(r"[,배원원%x배\s]","",str(raw)).strip()
        if c.lower() in SKIP: return None
...
def get_dynamic_watchlist() -> dict:
    ...
    try:
        portfolio = st.session_state.get("portfolio") or {}
        # [Fix] 메타키(__ai_opinions__) 순회로 인한 AttributeError 방지를 위해 ACC_MAP만 순회
        for key in ACC_MAP.values():
            for info in portfolio.get(key, {}).values():
                tk = info.get("ticker","")
                nm = info.get("name","")
...
"""
    ...
    # [Fix #1] 이전 분석 투자의견 로드 → 의견 일관성 실질 보장
...
  ✗ 수급 변화만 있고 펀더멘털 변화 없는 경우 (투자의견 변경 불가)
  ✗ 시장 전체 조정에 따른 동반 하락 (베타 효과)

  🚨 [최우선 예외 규칙: 서킷 브레이커 발동]
  ✓ 과거 투자의견(__ai_opinions__)을 존중하되, 오늘 수집된 실시간 뉴스에서 '치명적 거시 경제 위기(전쟁, 팬데믹 등)'가 감지되거나 개별 주가가 MA60 대비 -10% 이상 폭락한 경우, 기존의 모든 원칙과 과거 의견을 즉각 폐기하고 "구조적 리스크 발생: 전면 재검토 및 매도/비중축소"를 강력히 경고할 것.
👏 종합 평가
