...
def call_gemini(api_key, model_name, prompt, max_tokens,
    ...
    while truncated and cont_count<max_cont:
        ...
        _written   = [s for s in _allowed if any(s in full_text for s in [s])]
...
⑩ ⛔ 섹션 범위 엄수 — 이 리포트는 ## 4 ~ ## 8 이 전부이며 ## 9 이후 추가 섹션 생성을 절대 금지한다. (이어받기 시에도 동일 적용. 단, 이 종료 선언 지시문이나 금지 규칙 텍스트 자체를 리포트 본문에 그대로 출력하거나 노출하지 마십시오. 리포트는 오직 '✅ 분석 완료' 문자열로만 깔끔하게 끝나야 합니다.)
