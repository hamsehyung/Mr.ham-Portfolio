...
def _naver_price(ticker):
    ...
    # 1순위: 네이버 모바일 API (수급용으로 만들어둔 강력한 우회 세션 적용)
    for ep in ["basic", "integration"]:
        try:
            r = _NAVER_INV_SESSION.get(f"https://m.stock.naver.com/api/stock/{ticker}/{ep}", timeout=3)
            if r.status_code == 200:
                d  = r.json()
    ...
    # 2순위: 네이버 PC 실시간 폴링 API (해외 IP 차단이 거의 없는 무적 엔드포인트)
    try:
        r = _http.get(f"https://polling.finance.naver.com/api/realtime/domestic/stock/{ticker}", timeout=3)
        if r.status_code == 200:
            d = r.json().get("datas", [{}])[0]
            curr = ti(d.get("closePrice"))
            if curr > 0:
                # 폴링 API에서 제공하는 52주 고/저점 추출
                high = ti(d.get("high52wPrice", 0))
                low  = ti(d.get("low52wPrice",  0))
                return curr, high, low
    except: pass
