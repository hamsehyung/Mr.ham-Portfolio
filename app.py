...
_MODULE_SPEC = {
    "mrham_screener": ("SCREENER", [
        "run_screening", "build_candidate_context"]),
    "mrham_dart":     ("DART", [
...
def _load_module(modname: str, required: list):
    """모듈을 임포트하고 필수 함수 존재까지 확인. (모듈객체, 사용가능여부)"""
    try:
        mod = __import__(modname)
    except Exception:
        _MODULE_STATUS[modname] = "missing"
        return None, False
    absent = [f for f in required if not hasattr(mod, f)]
    if absent:
...
def run_full_screening(top_n: int = 15, momentum_pool: int = 250) -> dict:
    ...
    try:
        ...
        return SCREENER.run_screening(
            ...
            fund_fetcher=fetch_naver_fundamentals,
            fund_pool=300,
        )
    except Exception as e:
