...
def _naver_price(ticker):
    ...
    # 네이버가 차단된 해외 클라우드 환경을 위한 '다음(Daum) 금융 API' 우회 수집
    try:
        url = f"https://finance.daum.net/api/quotes/A{ticker}"
        headers = {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36",
            "Referer": "https://finance.daum.net/"
        }
        r = _http.get(url, headers=headers, timeout=3)
        if r.status_code == 200:
            d = r.json()
            curr = ti(d.get("tradePrice"))
            if curr > 0:
                high = ti(d.get("high52wPrice"))
                low  = ti(d.get("low52wPrice"))
