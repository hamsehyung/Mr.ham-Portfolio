...
#  Layer 0 — 전종목 유니버스 수집
# ═══════════════════════════════════════════════════════════
# 우선주 종목명 패턴: "삼성전자우", "현대차2우B", "대신증권우(전환)" 등
_PREF_NAME_RE = re.compile(r"(\d*우[A-C]?(\(전환\))?|우선주)$")
...
def _is_preferred_stock(name: str, ticker: str) -> bool:
    ...
    [v11.3 수정] 기존엔 '코드 끝자리 != 0' 이면 무조건 우선주로 판정했다.
      → 끝자리가 0이 아닌 보통주·ETF 가 대량 오탈락 (실측 100종목 중 90종목 탈락).
      → 종목명 패턴을 1순위 근거로 삼고, 코드 규칙은 이름이 우선주처럼 보일 때만 보조로 사용.
    """
    nm = str(name or "").strip()

    # ① 종목명이 우선주 패턴이면 우선주 (가장 신뢰도 높음)
    if _PREF_NAME_RE.search(nm):
        return True

    # ② 이름이 보통주인데 코드 끝자리만 다른 경우는 오탈락 위험이 커
    #    ETF/ETN/리츠 등은 제외하고 판단하지 않는다.
    return False
...
def normalize_universe(df: pd.DataFrame, source: str = "") -> pd.DataFrame:
    ...
    out["source"] = source
    # [v11.3] 신형 ETF/ETN 코드는 영숫자 6자리(0155M0, 0186L0).
    #   기존 ^\d{6}$ 는 이들을 유니버스에서 통째로 제거했다.
    out["code"] = out["code"].str.upper()
    out = out[out["code"].str.match(r"^[0-9A-Z]{6}$", na=False)]
...
def apply_hardcut(df: pd.DataFrame, rules: Optional[dict] = None) -> tuple[pd.DataFrame, dict]:
    ...
    # ── PER / PBR ────────────────────────────────────────────
    # [v11.3 핵심수정] 기존엔 결측(NaN)을 곧바로 탈락시켰다.
    #   FinanceDataReader.StockListing('KRX') 은 PER·PBR·EPS·BPS 를
    #   아예 반환하지 않으므로 전 종목이 NaN → 하드컷 통과 0개가 됐다.
    #   → 정책 변경: '데이터가 있는 종목만' 기준을 적용하고,
    #     결측 종목은 통과시킨 뒤 Layer 1.5(enrich_fundamentals)에서 보강한다.
    #     실제 부적격은 데이터 확보 후 걸러진다.
    _per = pd.to_numeric(cur["per"], errors="coerce")
    if _per.notna().any():
        before = len(cur)
        bad = _per.notna() & ((_per <= r["per_min"]) | (_per > r["per_max"]))
        cur = cur[~bad]
        stats["drop_per"] = before - len(cur)
    else:
        stats["drop_per"] = 0
        stats["per_missing"] = True      # 호출부에서 보강 필요 여부 판단용

    _pbr = pd.to_numeric(cur["pbr"], errors="coerce")
    if _pbr.notna().any():
        before = len(cur)
        bad = _pbr.notna() & (_pbr <= r["pbr_min"])
        cur = cur[~bad]
        stats["drop_pbr"] = before - len(cur)
    else:
        stats["drop_pbr"] = 0
        stats["pbr_missing"] = True
...
def enrich_momentum(df: pd.DataFrame, max_stocks: int = 400,
    ...
    if len(rest) > 0:
        # [v11.3] dtype 을 명시해 pandas FutureWarning(all-NA concat) 회피
        rest["momentum"] = pd.Series([np.nan] * len(rest),
                                     index=rest.index, dtype="float64")
        target["momentum"] = pd.to_numeric(target["momentum"], errors="coerce")
...
def enrich_fundamentals(df: pd.DataFrame, max_stocks: int = 300,
                        fund_fetcher=None) -> pd.DataFrame:
    """
    [v11.3 신규] Layer 1.5 — 하드컷 통과 종목에만 PER/PBR/ROE 를 보강.

    배경: 유니버스 소스(FDR StockListing)가 펀더멘털을 제공하지 않아
          가치·퀄리티 팩터가 통째로 죽어 있었다.
          전종목(2,700) 개별 조회는 비현실적이므로,
          시총 상위 max_stocks 개에만 조회를 건다.

    fund_fetcher: (ticker) -> {"per":float, "pbr":float, "roe":float, "eps":.., "bps":..}
                  app.py 의 fetch_naver_fundamentals 를 주입해 재사용한다.
    """
    if df is None or len(df) == 0 or fund_fetcher is None:
        return df

    out = df.copy()
    # 이미 값이 있으면 건너뛰고, 없는 것만 채운다
    need_mask = pd.to_numeric(out.get("per"), errors="coerce").isna()
    if "pbr" in out.columns:
        need_mask |= pd.to_numeric(out["pbr"], errors="coerce").isna()

    # 시총 큰 순으로 우선 보강 (조회 비용 제한)
    targets = (out[need_mask].sort_values("marcap", ascending=False)
               .head(max_stocks)["code"].tolist())
    if not targets:
        return out
    ...
    for field in ("per", "pbr", "roe", "eps", "bps"):
        vals = out[field].tolist() if field in out.columns else [None] * len(out)
        for i, code in enumerate(out["code"]):
            cur = vals[i]
            if (cur is None or (isinstance(cur, float) and math.isnan(cur))) and code in got:
                v = got[code].get(field)
                if v is not None:
                    try:
                        vals[i] = float(v)
                    except Exception:
                        pass
        out[field] = pd.to_numeric(pd.Series(vals, index=out.index), errors="coerce")

    # ROE 재유도 (EPS/BPS 가 새로 채워졌을 수 있음)
    _roe = pd.to_numeric(out["roe"], errors="coerce")
    _m = _roe.isna() & out["bps"].notna() & (out["bps"] > 0) & out["eps"].notna()
    _roe[_m] = out.loc[_m, "eps"] / out.loc[_m, "bps"] * 100
    out["roe"] = _roe
    return out
...
def run_screening(top_n: int = 15,
    ...
    # ── Layer 1.5: 펀더멘털 보강 (PER/PBR/ROE) ──
    # 유니버스 소스가 펀더멘털을 안 주는 경우를 여기서 메운다.
    if fund_fetcher is not None:
        filtered = enrich_fundamentals(filtered, max_stocks=fund_pool,
                                       fund_fetcher=fund_fetcher)
        # 보강 후 PER/PBR 부적격을 다시 걸러낸다
        filtered, stats2 = apply_hardcut(filtered)
        stats["after_fundamentals"] = stats2["output"]
        stats["drop_per_final"] = stats2.get("drop_per", 0)
        stats["drop_pbr_final"] = stats2.get("drop_pbr", 0)
        if len(filtered) == 0:
            return {"top": pd.DataFrame(), "all": pd.DataFrame(),
                    "stats": stats, "source": source, **meta,
                    "error": "펀더멘털 보강 후 통과 종목 0개 — 기준을 완화하세요."}

    stats["fund_available"] = int(pd.to_numeric(
        filtered.get("per"), errors="coerce").notna().sum()) if "per" in filtered else 0
