...
# [v11.2] 모듈 버전 — app.py 가 기능 지원 여부를 확인하는 데 사용.
# app.py 만 갱신하고 이 파일을 갱신하지 않으면 버전 불일치가 발생하므로,
# app.py 쪽에서 이 값과 hasattr 로 방어한 뒤 안내 메시지를 띄운다.
MODULE_VERSION = "11.2"
...
def validate_api_key(api_key: str, session=None) -> dict:
    """
    DART API 키의 유효성을 서버에 실제로 물어본다.
    형식만 보고 통과시키면 아무 문자열이나 '연동됨'으로 표시되는 문제가 생긴다.

    Returns: {"valid": bool, "status": str, "message": str}
      status — ok | bad_format | invalid_key | no_permission | network | error
    """
    key = (api_key or "").strip()

    # ── 1차: 형식 검사 (네트워크 낭비 방지) ──
    if not key:
        return {"valid": False, "status": "bad_format", "message": "키가 비어 있습니다."}
    if not re.fullmatch(r"[0-9a-f]{40}", key):
        return {"valid": False, "status": "bad_format",
                "message": f"DART 키는 40자리 소문자+숫자입니다. (입력: {len(key)}자)"}
    if not HAS_REQUESTS:
        return {"valid": False, "status": "error", "message": "requests 미설치"}

    # ── 2차: 서버 실제 조회 ──
    # DART 응답 코드
    #   000 정상 / 013 조회결과없음(키는 유효) / 010 미등록키
    #   011 사용중지 / 012 접근불가IP / 020 한도초과 / 100 파라미터오류
    sess = session or requests
    today = today_kst()
    try:
        r = sess.get(DART_LIST_URL, params={
            "crtfc_key": key,
            "bgn_de": (today - datetime.timedelta(days=1)).strftime("%Y%m%d"),
            "end_de": today.strftime("%Y%m%d"),
            "page_count": 1, "page_no": 1,
        }, timeout=8)
    except Exception as e:
        return {"valid": False, "status": "network",
                "message": f"DART 서버에 연결할 수 없습니다 ({type(e).__name__})"}

    if r.status_code != 200:
        return {"valid": False, "status": "network",
                "message": f"DART 서버 응답 오류 (HTTP {r.status_code})"}

    try:
        d = r.json()
    except Exception:
        return {"valid": False, "status": "error",
                "message": "DART 응답을 해석할 수 없습니다."}

    code = str(d.get("status", ""))
    if code in ("000", "013"):
        return {"valid": True, "status": "ok", "message": "유효한 키입니다."}

    return {"valid": False, "status": {
        "010": "invalid_key", "011": "invalid_key",
        "012": "no_permission", "020": "no_permission",
    }.get(code, "error"), "message": {
        "010": "등록되지 않은 키입니다. 발급받은 키를 다시 확인해주세요.",
        "011": "사용 중지된 키입니다. DART에서 재발급받아주세요.",
        "012": "이 서버 IP에서 접근이 거부되었습니다.",
        "020": "일일 요청 한도(20,000건)를 초과했습니다.",
        "100": "요청 형식 오류입니다.",
    }.get(code, f"DART 오류 {code}: {d.get('message','')}")}
