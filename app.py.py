...
# 수급 API 전용 세션 — [C1 수정] @st.cache_resource 로 감싸서
# 프로세스당 1회만 초기화 (매 Streamlit rerun 시 최대 4초 블로킹 제거)
@st.cache_resource
def _init_naver_inv_session() -> requests.Session:
    s = requests.Session()
    s.headers.update({
    ...
    try:
        s.get("https://m.stock.naver.com/", timeout=4)  # 쿠키 초기 획득 (1회)
    except Exception:
        pass
    return s

_NAVER_INV_SESSION = _init_naver_inv_session()
...
def fetch_watchlist_prices(watchlist_tuple: tuple) -> dict:
    ...
    with ThreadPoolExecutor(max_workers=min(8, len(watchlist_tuple))) as ex:
